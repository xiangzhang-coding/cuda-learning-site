...
// [ex16-initcheck-corrected-start]
namespace {
...
int main() {
  ...
  if (!cuda_ok(
      ...
      !cuda_ok(
          ...
          "cudaMalloc(output)") ||
      !cuda_ok(cudaMemset(device_input, 0, kBytes), "cudaMemset(input)")) {
    return EXIT_FAILURE;
  }
  ...
  std::cout << "scenario=" << ex16::kScenarios[5].id
            << " result=" << (passed ? "PASS" : "FAIL") << '\n';
  return passed ? EXIT_SUCCESS : EXIT_FAILURE;
}
// [ex16-initcheck-corrected-end]
