...
The current Stable Curriculum contains 44 Learning Units: O01-O08, F01-F08, M01-M19, A01-A04, and Q01-Q05. Exercises and reviewed solutions accompany O02-O08, F01-F08, M01-M19, A01-A04, and every Q unit. The current public surface also contains fourteen Runnable Examples, EX01-EX13 and EX16; six noncontiguous Labs, LAB01-LAB05 and LAB07, with no LAB06 destination; fourteen formal Visual Explainers, comprising standalone VIS01-VIS10 plus embedded VIS19-VIS22; 45 Practice Bank entries; 139 Glossary terms; 56 source/version records; and About. These files form exactly 167 bilingual Publication Pairs and 334 source routes. EX14 and EX15 remain absent.

The strict graph extends the reviewed R1 graph with `M09<-[M07,M08]`, `M10<-[M01,M02]`, `M11<-[M07,M08]`, `M12<-[M05,M06]`, `M13<-[M03,M05,M08]`, `M14<-[M07,M08]`, `M15<-[F04,O04]`, `M16<-[M15,F06]`, `M17<-[M16,F06]`, `M18<-[M15,M16]`, `M19<-[O04,M15]`, `A01<-[F03,F04,M02]`, `A02<-[M03,M05,M06]`, `A03<-[A02,M05]`, `A04<-[M03,M05]`, `Q02<-[Q01,A02]`, `EX07<-[M07,M08,M09]`, `EX08<-M10`, `EX09<-M14`, `EX10<-[M15,M16]`, `EX11<-[A02,Q02]`, `EX12<-A03`, `EX13<-A04`, `VIS09<-[M15,M16,M17]`, and `VIS10<-A02`. F08 remains related to LAB03 but is not a LAB03 prerequisite. The validated current catalog contains 6 Labs, 45 Practice Bank entries, 14 Visual Explainers, 139 Glossary terms, and 56 source/version records, or 260 records total.
...
R1 declares no Reference Environment or Runtime-Verified subject. EX02 and LAB02 retain the only qualifying Compile-Checked evidence from R1. The current publication additionally makes EX10 Compile-Checked from retained run 33275734951. Thirteen current Runnable Examples, EX01-EX09, EX11-EX13, and EX16, and all six current Labs remain Pending Hardware Verification at runtime. EX10 is Runtime-Not-Applicable because its acceptance contract generates and inspects artifacts without executing the final host artifact or any GPU executable. EX07-EX09 and EX11-EX13 retain empty compilation evidence and no Compile-Checked claim. The current publication declares no Reference Environment, Runtime-Verified subject, or performance observation. In particular, it publishes no measured overlap, migration, graph, reduction, scan, histogram, or contention performance, and no timing, speedup, profiler, or runtime result for the new examples. Host-only tests, source review, expected observations, static fallbacks, and browser models do not upgrade those evidence axes.
...
Host-only tests are available without CUDA for the thirteen current Runnable Examples whose runtime remains Pending Hardware Verification:
...
EX07's host test checks deterministic chunk coverage and its CPU reference, EX08's checks the declared access sequence and CPU oracle, and EX09's checks the fixed DAG and replay oracle. EX11 checks its double CPU reference and tolerance contract, EX12 checks exact prefix recurrences and totals, and EX13 checks exact bins and the sum-of-bins invariant. They execute no CUDA, establish no copy/compute overlap, managed-memory migration, CUDA Graph execution, reduction, scan, histogram, contention, or performance observation, and grant no Compile-Checked or runtime Evidence Status. Their project manifests retain empty compilation and recorded-observation arrays plus Pending Hardware Verification runtime.
...
[Issue #19](https://github.com/xiangzhang-coding/cuda-learning-site/issues/19) refreshed the M09-M14, EX07-EX09, and VIS08 teaching coordinates. The M15-M19, EX10, and VIS09 refresh used Context7 only for discovery and cross-checking; exact NVIDIA owner documentation and archives in `SRC-CUDA-031` through `SRC-CUDA-035` govern those facts. [Issue #21](https://github.com/xiangzhang-coding/cuda-learning-site/issues/21) reviewed exact current and archived CUDA owner documentation, pinned CCCL v3.4.2 owner source, and Blelloch's original scan report recorded in `SRC-HIST-003` and `SRC-CUDA-036` through `SRC-CUDA-040` on 2026-08-30. Context7 output is not itself a source or Evidence Status record. The refresh records no measured overlap, migration, graph launch overhead, reduction, scan, histogram, contention, concurrency, timing, or speedup.

The build is fully static. All fourteen Visual Explainers use deterministic browser-only models, preserve static or textual fallbacks, execute no CUDA, and grant no CUDA Evidence Status. VIS09's host/device branches, target plans, and artifact flow are original teaching composition rather than compiler execution or artifact observation. VIS10's eight-lane reduction-stage ledger and static stage-diagram fallback are deterministic teaching composition rather than runtime or performance observations. Theme selection is the only learner preference retained across browser sessions. The website has no server application, account, progress tracking, API, hosted GPU service, or in-browser CUDA execution.
