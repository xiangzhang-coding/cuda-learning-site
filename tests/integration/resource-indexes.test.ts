...
describe('published resource indexes', () => {
  ...
  it('keeps all eighty-nine bilingual Practice Bank entries complete and nonduplicative', async () => {
    const practiceIds = RESOURCE_INDEX_RECORDS
      .filter(({ group }) => group === 'practice')
      .map(({ planningId }) => planningId);
    expect(practiceIds).toHaveLength(111);
  ...
  it('publishes no planned placeholder and keeps exact eligible populations', async () => {
    ...
    expect(counts).toEqual({ labs: 18, practice: 111, visuals: 21, glossary: currentPublication.scope.glossaryTerms, sources: 115 });
    ...
    for (const planningId of [
      'LAB10', 'PB-R3-007', 'PB-R3-008', 'PB-R3-009', 'PB-R3-010',
      'SRC-CUDA-053', 'SRC-CUDA-054', 'SRC-CUDA-055',
    ]) {
      const record = RESOURCE_INDEX_RECORDS.find((candidate) => candidate.planningId === planningId);
    ...
    for (const planningId of [
      'PB-R3-011', 'PB-R3-012', 'PB-R3-013', 'PB-R3-014', 'VIS18',
      'TERM-166', 'TERM-167', 'TERM-168', 'TERM-169', 'TERM-170',
      'SRC-CUDA-056', 'SRC-CUDA-057', 'SRC-CUDA-058',
    ]) {
      const record = RESOURCE_INDEX_RECORDS.find((candidate) => candidate.planningId === planningId);
    ...
    for (const planningId of [
      ...
      if (planningId.startsWith('SRC-')) expect(record?.sourceAccessDate, planningId).toBe('2026-09-04');
    }

    for (const planningId of [
      'LAB11', 'PB-R4-003', 'PB-R4-004', 'PB-R4-005', 'PB-R4-006',
      'TERM-183', 'TERM-184', 'TERM-185', 'TERM-186',
      'SRC-CUDA-063', 'SRC-CUDA-064', 'SRC-CUDA-065', 'SRC-CUDA-066',
    ]) {
      const record = RESOURCE_INDEX_RECORDS.find((candidate) => candidate.planningId === planningId);
  ...
  it('projects every Lab title and Evidence Status exactly and grants Visual Explainers no CUDA status', async () => {
    ...
    for (const route of ['/visuals/', '/en/visuals/']) {
      ...
      expect(document.querySelectorAll('[data-resource-card] [data-no-evidence]').length).toBe(21);
