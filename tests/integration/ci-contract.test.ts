...
describe('GitHub Actions quality contract', () => {
  ...
  it('fails CI on flaky browser behavior and retains reviewed cross-browser evidence', async () => {
    ...
    expect(workflow).toMatch(
      /e2e-cross-browser:[\s\S]*?runs-on: ubuntu-24\.04\s+(?:#[^\n]*\n\s+)?timeout-minutes: 45/,
    );
    expect(workflow).toMatch(
    ...
    expect(crossBrowser).toMatch(/strategy:\s+fail-fast: false\s+matrix:\s+shard: \[1, 2, 3\]/);
    expect(crossBrowser).toContain('npm run test:e2e:cross-browser -- --shard=${{ matrix.shard }}/3');
