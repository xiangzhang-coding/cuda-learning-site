...
describe('CUDA compile evidence workflow', () => {
  ...
  it('records manifests, scans retained evidence, and gates all matrix jobs', async () => {
    ...
    expect(workflow).toContain('EX19_BUILD_RESULT: ${{ needs.ex19-build.result }}');
    expect(workflow).toContain('EX20_BUILD_RESULT: ${{ needs.ex20-build.result }}');
    expect(workflow).toContain('EX21_PYTHON_BUILD_RESULT: ${{ needs.ex21-python-build.result }}');
    expect(workflow).toContain(
      'needs: [cuda-compile, ex01-build, ex03-build, ex04-build, ex05-build, ex06-build, ex07-build, ex08-build, ex09-build, ex10-compile, ex11-build, ex12-build, ex13-build, ex14-build, ex15-build, ex16-build, ex17-build, ex18-build, ex19-build, ex20-build, ex21-python-build]',
    );
    ...
    expect(workflow).toContain('if [ "$EX19_BUILD_RESULT" != "success" ]; then exit 1; fi');
    expect(workflow).toContain('if [ "$EX20_BUILD_RESULT" != "success" ]; then exit 1; fi');
    expect(workflow).toContain('if [ "$EX21_PYTHON_BUILD_RESULT" != "success" ]; then exit 1; fi');
