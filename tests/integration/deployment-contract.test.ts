...
describe('Cloudflare assets-only deployment contract', () => {
  ...
  it('emits both source identities and complete project and bundled-interface notices without a Worker application', async () => {
    ...
    expect(publication.scope.learningUnits).toHaveLength(100);
    ...
    expect(publication.scope.labs).toHaveLength(17);
    ...
    expect(publication.scope.labs.length + publication.scope.practiceBankEntries + publication.scope.visualExplainers.length
      + publication.scope.glossaryTerms + publication.scope.sourceRecords).toBe(465);
    expect(publication.evidence.pendingHardwareVerification).toHaveLength(40);
  ...
  it('publishes the selected Wrangler authority plus reviewed Workers Builds, preview, smoke, and rollback boundaries', async () => {
    ...
    for (const value of ['2026-09-15', '95 Learning Units', '23 Runnable Examples', '102 Practice Bank entries',
      '207 Glossary terms', '108 source records', '453 catalog records', '345 Publication Pairs', '690 source routes',
      '94 Exercise-set and 94 solution-set Publication Pairs']) {
    ...
    expect(deployment).toContain('38 Pending Hardware Verification subjects');
    ...
    expect(deployment).toContain('all 690 routes');
  ...
  it('accepts only the production origin, a Cloudflare Preview URL, or loopback in explicit local mode', async () => {
    ...
    ).resolves.toMatchObject({ stdout: expect.stringMatching(/Total: \d+ tests in 1 file/) });
    await expect(
