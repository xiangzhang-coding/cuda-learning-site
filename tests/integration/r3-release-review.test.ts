...
const currentRunnableExamples = [...runnableExamples, 'EX17', 'EX18', 'EX19', 'EX20'] as const;
...
describe('R3 release review', () => {
  it('publishes the exact immutable R3 contract and advances the rolling boundary to R4', async () => {
    ...
    expect(currentManifest.compatibility.componentBoundaries.cusparse).toEqual({
      bundled: [
    ...
    expectExactMembers(destinationIds(/^L\d{2}$/), ['L01', 'L02', 'L03', 'L04', 'L05', 'L06', 'L07', 'L08', 'L09', 'L10', 'L11', 'L12', 'L13']);
    ...
    expect(recordsByGroup.practice).toHaveLength(82);
    expect(recordsByGroup.visuals).toHaveLength(19);
    expect(recordsByGroup.glossary).toHaveLength(196);
    expect(recordsByGroup.sources).toHaveLength(92);
    expect(RESOURCE_INDEX_RECORDS).toHaveLength(401);
    expect(publishedRoutes).toHaveLength(554);
    expect(new Set(publishedRoutes).size).toBe(554);
  ...
  it('documents the completed R3 review without pre-certifying dynamic acceptance', async () => {
    ...
    for (const practice of [zhPractice, enPractice]) {
      ...
      for (const id of ['PB-R4-001', 'PB-R4-002', 'PB-R4-003', 'PB-R4-004', 'PB-R4-005', 'PB-R4-006', 'PB-R4-007', 'PB-R4-008', 'PB-R4-009', 'PB-R4-010', 'PB-R4-011', 'PB-R4-012', 'PB-R4-013', 'PB-R4-014', 'PB-R4-015', 'PB-R4-016']) expect(practice).toContain(id);
    }
  });
});
