...
for (const locale of ['', 'en/']) {
  test(`${locale || 'zh-CN/'}Python bridge follows P01 to P03 and EX21 without downstream placeholders`, async ({ page, baseURL }) => {
    ...
    expect(await page.locator('nav a[href*="/triton/"]').evaluateAll((links) => links.map((link) => link.getAttribute('href')).sort()))
      .toEqual(['autotuning', 'blocked-matrix-multiplication', 'debugging', 'fused-softmax', 'masked-vector-addition', 'persistent-kernels', 'programs-and-block-values'].map((slug) => `/${locale}triton/${slug}/`));
