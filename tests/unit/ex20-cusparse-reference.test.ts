...
it('checks the double CSR oracle against both literal FP32 fixtures, including the empty row and beta', async () => {
  const directory = await mkdtemp(path.join(tmpdir(), 'ex20-host-'));
  try {
