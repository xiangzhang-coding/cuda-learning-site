...
it('gates all three EX20 profiles without GPU access and requires scanned artifacts before upload', async () => {
  const workflow = parseFrontmatter(`---\n${await readFile(
    path.join(root, '.github/workflows/cuda-compile.yml'), 'utf8')}\n---`).frontmatter;
  const project = JSON.parse(await readFile(path.join(root, 'examples/ex20-cusparse-spmv/project.json'), 'utf8'));
  const job = workflow.jobs['ex20-build'];
  ...
  expect(builds[0].run).toContain('--workdir /workspace/examples/ex20-cusparse-spmv');
  ...
  const artifactPath = 'artifacts/cuda-ex20/${{ matrix.profile }}';
  ...
  expect(gate.needs).toContain('ex20-build');
  expect(gate.if).toBe('${{ always() }}');
  const step = gate.steps[0];
  expect(step.env.EX20_BUILD_RESULT).toBe('${{ needs.ex20-build.result }}');
  for (const result of ['success', 'failure', 'cancelled', 'skipped']) {
    const run = spawnSync('bash', ['-c', step.run], {
      encoding: 'utf8', env: { ...process.env,
        ...Object.fromEntries(Object.keys(step.env).map((key) => [key, 'success'])), EX20_BUILD_RESULT: result },
    });
