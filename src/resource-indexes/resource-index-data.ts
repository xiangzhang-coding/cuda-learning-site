...
const labs: readonly ResourceIndexRecord[] = [
  ...
  {
    planningId: 'LAB13', group: 'labs',
    title: PUBLISHED_DESTINATIONS.LAB13.title, href: PUBLISHED_DESTINATIONS.LAB13.href,
    resourceType: 'guided-lab', difficulty: 'advanced', prerequisites: ['P08', 'P09'],
    relatedUnits: ['P10', 'EX22'],
    hardwareGate: localized('原生 Linux；一个 CC 8.0+ GPU，至少 8 GB；构建与 CPU 检查不需要 GPU。', 'Native Linux; one CC 8.0+ GPU with at least 8 GB; build and CPU checks need no GPU.'),
...
const practice: readonly ResourceIndexRecord[] = [
  ...
  },
  {
    ...
    group: 'practice',
    title: localized('审查发布协议的作用域、内存序（Memory Order）与系统作用域假设', 'Audit a scope/order publication protocol and system-scope memory assumptions'),
    href: localized('/practice/#pb-r4-005', '/en/practice/#pb-r4-005'),
    resourceType: 'evidence-review',
    difficulty: 'advanced',
    prerequisites: ['L05'],
    relatedUnits: ['M05', 'M19', 'L05'],
    hardwareGate: localized('无；仅审查静态协议。未来执行需原生 Linux（Native Linux）和 CC 7.5+；CPU/GPU 共享内存还须核实所分配内存的可访问性（accessibility）及原子操作支持。', 'None; static protocol review. Future execution requires native Linux and CC 7.5+; CPU/GPU sharing separately requires allocation accessibility and atomic support.'),
    versionGate: localized('选定 CCCL/libcu++ v3.4.2；仅采用 Toolkit 12.9.2/13.3.1 的 C++17/C++20；只讨论 32 位原子类型，不把系统作用域（system scope）当作共享内存已受支持的证明。', 'Selected CCCL/libcu++ v3.4.2; C++17/C++20 in Toolkits 12.9.2/13.3.1 only; 32-bit atomic types only, with no shared-memory support inferred from system scope.'),
    reviewedOn: '2026-09-05',
  ...
  },
  {
    ...
    group: 'practice',
    title: localized('审查 libcu++ 的可移植性、阶段所有权与对齐回退方案', 'Audit libcu++ portability, stage ownership, and an alignment fallback'),
    href: localized('/practice/#pb-r4-006', '/en/practice/#pb-r4-006'),
    resourceType: 'evidence-review',
    difficulty: 'advanced',
    prerequisites: ['L05'],
    relatedUnits: ['M05', 'M13', 'M19', 'L05'],
    hardwareGate: localized('无；仅做静态审查。SM 70 是 API 的能力下限，本站与 CCCL 的支持下限为 CC 7.5；CC 8.0+ 即使符合 cp.async 条件，也不证明重叠（overlap）或加速（speedup）。', 'None; static review. SM 70 is the API floor, while the site/CCCL floor is CC 7.5; eligible cp.async on CC 8.0+ proves no overlap or speedup.'),
  ...
  {
    planningId: 'PB-R5-001',
    group: 'practice',
    title: localized('审查排队请求的桥接所有权', 'Audit bridge ownership for queued requests'),
    href: localized('/practice/#pb-r5-001', '/en/practice/#pb-r5-001'),
    resourceType: 'correctness-debugging',
    difficulty: 'intermediate',
    prerequisites: ['P01'],
    relatedUnits: ['F04', 'M07', 'P01', 'EX21'],
    hardwareGate: localized('无；原创纸面队列与所有权审查，不执行 CUDA。', 'None; original paper queue and ownership audit, without CUDA execution.'),
    versionGate: localized('普通 CPython 3.14.7；core 1.2.0、bindings 13.4.1、pathfinder 1.8.1、NumPy 2.5.3；唯一原生 Linux Toolkit 13.3.1 配置。', 'Ordinary CPython 3.14.7; core 1.2.0, bindings 13.4.1, pathfinder 1.8.1, NumPy 2.5.3; one native Linux Toolkit 13.3.1 profile.'),
    reviewedOn: '2026-09-12',
    keywords: localized('CUDA Python 主机编排 借用视图 所有权 队列 完成 数据快照', 'CUDA Python host orchestration borrowed view ownership queue completion snapshot'),
  },
  {
    planningId: 'PB-R5-002',
    group: 'practice',
    title: localized('审查标量位宽与过早释放', 'Audit scalar width and premature release'),
    href: localized('/practice/#pb-r5-002', '/en/practice/#pb-r5-002'),
    resourceType: 'correctness-debugging',
    difficulty: 'intermediate',
    prerequisites: ['P02'],
    relatedUnits: ['F07', 'P02', 'EX21'],
    hardwareGate: localized('无；纸面 ABI、容量与生命周期审查。运行另需原生 Linux、受编译器支持的 CC 7.5+ GPU。', 'None; paper ABI, capacity and lifetime audit. Runtime separately needs native Linux and a compiler-supported CC 7.5+ GPU.'),
    versionGate: localized('core 1.2.0 的 ctypes 标量位宽、显式复制目标、sync 与共享主上下文合同；EX21 唯一 Toolkit 13.3.1 配置。', 'Core 1.2.0 ctypes scalar widths, explicit copy destinations, sync and shared primary-context contracts; EX21 single Toolkit 13.3.1 profile.'),
    reviewedOn: '2026-09-12',
    keywords: localized('ctypes uint32 float ABI Buffer 字节 尾部 NaN 哨兵 全量比较 主上下文 sync close', 'ctypes uint32 float ABI Buffer bytes tail NaN sentinel full comparison primary context sync close'),
  },
  {
    planningId: 'PB-R5-003',
    group: 'practice',
    title: localized('拆分编译与链接的错误接口', 'Separate compiler and linker error surfaces'),
    href: localized('/practice/#pb-r5-003', '/en/practice/#pb-r5-003'),
    resourceType: 'correctness-debugging',
    difficulty: 'intermediate',
    prerequisites: ['P03'],
    relatedUnits: ['M15', 'M16', 'P03', 'EX21'],
    hardwareGate: localized('无；纸面错误与产物审查。实际构建无需 GPU，但需要真实 libcuda 用户态库及选定原生编译器/链接器。', 'None; paper error and artifact audit. Actual build needs no GPU but requires real libcuda userspace and the selected native compiler/linker.'),
    versionGate: localized('bindings 13.4.1：稳定 NVRTC 状态元组与 nvJitLink 异常；core 1.2.0；NVRTC/nvJitLink 13.3.33。', 'Bindings 13.4.1: stable NVRTC status tuples versus nvJitLink exceptions; core 1.2.0; NVRTC/nvJitLink 13.3.33.'),
    reviewedOn: '2026-09-12',
  ...
  {
    planningId: 'PB-R5-008', group: 'practice',
    title: localized('审查切片是否仍满足输入合同', 'Audit sliced input contracts'),
    href: localized('/practice/#pb-r5-008', '/en/practice/#pb-r5-008'),
    resourceType: 'correctness-debugging', difficulty: 'intermediate',
    prerequisites: ['P08'], relatedUnits: ['P08', 'EX22'], hardwareGate: noHardware,
    versionGate: localized('PyTorch 2.11.0+cu128；连续偏移、步长、空输入和新输出合同。', 'PyTorch 2.11.0+cu128; contiguous offsets, strides, empty input and fresh output.'),
    reviewedOn: '2026-09-13',
  },
  {
    planningId: 'PB-R5-009', group: 'practice',
    title: localized('区分三份算子验收报告', 'Distinguish three operator acceptance reports'),
    href: localized('/practice/#pb-r5-009', '/en/practice/#pb-r5-009'),
    resourceType: 'evidence-review', difficulty: 'advanced',
    prerequisites: ['P09'], relatedUnits: ['P09', 'EX22'], hardwareGate: noHardware,
    versionGate: localized('PyTorch 2.11.0+cu128；opcheck、gradcheck 和 fullgraph 不是同一证据。', 'PyTorch 2.11.0+cu128; opcheck, gradcheck and fullgraph supply distinct checks.'),
    reviewedOn: '2026-09-13',
  },
  {
    planningId: 'PB-R5-010', group: 'practice',
    title: localized('拒绝跨版本重用扩展 wheel', 'Reject cross-version extension wheel reuse'),
    href: localized('/practice/#pb-r5-010', '/en/practice/#pb-r5-010'),
    resourceType: 'evidence-review', difficulty: 'advanced',
  ...
  {
    planningId: 'PB-R5-015', group: 'practice',
    title: localized('识别 Softmax 零填充陷阱', 'Identify the softmax zero-padding trap'),
    href: localized('/practice/#pb-r5-015', '/en/practice/#pb-r5-015'),
    resourceType: 'correctness-debugging', difficulty: 'intermediate', prerequisites: ['T03'], relatedUnits: ['T03', 'LAB15'],
    hardwareGate: noHardware, versionGate: same('Triton 3.7.1'), reviewedOn: '2026-09-14',
  },
  {
    planningId: 'PB-R5-016', group: 'practice',
...
const glossary: readonly ResourceIndexRecord[] = [
  ...
  glossaryRecord('TERM-197', 'CUDA Python · CUDA Python 接口体系', 'kernel-vocabulary', ['P01', 'P02', 'P03', 'EX21'], 'core 1.2.0 与 bindings 13.4.1 独立固定；Python 主机编排不把任意 Python 函数编译到 GPU。', 'Core 1.2.0 and bindings 13.4.1 are independently pinned; Python host orchestration does not compile arbitrary Python functions to the GPU.', '2026-09-12'),
...
const sources: readonly ResourceIndexRecord[] = [
  ...
  sourceRecord(
    'SRC-CUDA-065',
    localized('L05 选定版本的 libcu++ 同步、阶段与异步复制合同', 'L05 selected libcu++ synchronization, phase, and async-copy contracts'),
    'cuda-version-record',
    ['M05', 'M13', 'L05'],
    localized(
      'CCCL/libcu++ v3.4.2，提交 d36012203ef73ac7f966e848dd88482273e91e02；核对该标签下的 atomic/atomic_ref、barrier、pipeline、memcpy_async 与 aligned_size_t。逐文件检查头文件和测试中的 Apache-2.0 WITH LLVM-exception 声明；文档没有单独声明。上游测试仅检查源码、未执行；没有复制或改编。',
  ...
  sourceRecord(
    'SRC-CUDA-066',
    localized('L05 libcu++ 兼容性与已知问题边界', 'L05 libcu++ compatibility and known-issue boundaries'),
    'cuda-version-record',
    ['M19', 'L05'],
    localized(
      '核对 CCCL v3.4.2 的 README、ci/matrix.yaml、2026-08-05 发布记录、仍未关闭的 #10967/#10499，以及 NVCC 12.9/13.3 的 Linux 支持政策；只评估 12.9.2/13.3.1 常规通道的 C++17/C++20，排除 11.8。12.X=12.9、13.X=13.2 不证明 13.3 已执行；未测试所有配置组合，也未复现问题。',
  ...
  sourceRecord(
    'SRC-CUDA-077',
    localized('CUDA core 1.2.0 API 与资源所有权', 'CUDA core 1.2.0 APIs and resource ownership'),
    'cuda-version-record',
    ['P01', 'P02', 'P03', 'EX21'],
    localized('固定源码 53b43746e501f1a0b627f951604991636f77cd9c；设备/主上下文、Buffer/ctypes、sync、Program/Linker 与延迟加载。PTX 编译查询驱动版本，不初始化 GPU。', 'Pinned source 53b43746e501f1a0b627f951604991636f77cd9c; device/primary context, Buffer/ctypes, sync, Program/Linker and lazy loading. PTX compilation queries driver version without GPU initialization.'),
    '2026-09-12', '2026-09-12',
  ),
  sourceRecord(
    'SRC-CUDA-078',
    localized('CUDA bindings 13.4.1 编译、链接与错误合同', 'CUDA bindings 13.4.1 compilation, linking and error contracts'),
    'cuda-version-record',
    ['P02', 'P03', 'EX21'],
    localized('固定源码 0770ab6ced8931ae8b6c6e5f622f48cb07ea99fa；稳定 NVRTC 状态元组、nvJitLink 异常/可写输出/日志所有权；排除实验性 v2。', 'Pinned source 0770ab6ced8931ae8b6c6e5f622f48cb07ea99fa; stable NVRTC status tuples, nvJitLink exceptions/writable outputs/log ownership; experimental v2 excluded.'),
    '2026-09-12', '2026-09-12',
  ),
  sourceRecord(
    'SRC-CUDA-079',
    localized('CUDA Python 单一配置、原生库、锁定与许可', 'CUDA Python single profile, native libraries, locks and licenses'),
    'cuda-version-record',
    ['P01', 'P02', 'P03', 'EX21'],
