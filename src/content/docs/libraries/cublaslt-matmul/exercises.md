...
**约束：** `op(A):3 x 2`，A 使用 T，实际行主序存储为 `2 x 3, ld=4`；B 行主序为 `2 x 5, ld=7`；C、D 为不重叠的行主序 `3 x 5`，主维度（leading dimension）分别 6、8。全部 FP32，计算选 `CUBLAS_COMPUTE_32F_PEDANTIC`、FP32 缩放类型、主机指针模式和默认收尾操作（epilogue）。不得把 C 当成可以随意转置的输入，也不得因 D 改为原位就只比较布局属性。
