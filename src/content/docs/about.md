...
截至 **2026-09-13**，当前发布有 87 个学习单元、22 个可运行示例（EX01-EX22）、14 个实验（LAB01-LAB14）、19 项可视化讲解、94 个[练习题库（Practice Bank）](/practice/)条目、204 个[术语表](/glossary/)词条、102 条[来源记录](/sources-and-versions/)，共 433 条资源目录记录、317 个双语发布对、634 条源路由，含 86 组练习与 86 组独立参考解答。
...
当前 94 道练习题库题目包括 PB-R5-001 至 PB-R5-012。R4 八道库与算法选择题、R3 十道 Nsight 报告分析题不变。只有 EX02、EX10、LAB02 编译已检查（Compile-Checked），EX10 无需运行验证（Runtime-Not-Applicable）。其余 21 个示例和全部 14 个实验共 35 个主体待硬件验证（Pending Hardware Verification）。P01-P12 四个证据数组为空。EX21 和 EX22 编译/已记录观察为空，主机检查不授予 GPU 证据。运行已验证（Runtime-Verified）、社区已观察（Community-Observed）、基准环境（Reference Environment）、性能观察均为零。六份分析器计划只有预期内容。
