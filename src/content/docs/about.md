...
截至 **2026-09-14**，当前发布有 92 个学习单元、23 个可运行示例（EX01-EX23）、16 个实验（LAB01-LAB16）、20 项可视化讲解、100 个[练习题库（Practice Bank）](/practice/)条目、207 个[术语表](/glossary/)词条、105 条[来源记录](/sources-and-versions/)，共 448 条资源目录记录、336 个双语发布对、672 条源路由，含 91 组练习与 91 组独立参考解答。[T04](/triton/blocked-matrix-multiplication/) 和 [T05](/triton/autotuning/) 在 Triton 心智模型/向量加法/Softmax 路径上增加分块 GEMM，配套 [LAB16](/labs/autotune-triton-gemm/)。
...
当前 100 道练习题库题目包括 PB-R5-001 至 PB-R5-018。R4 八道库与算法选择题、R3 十道 Nsight 报告分析题不变。只有 EX02、EX10、LAB02 编译已检查（Compile-Checked），EX10 无需运行验证（Runtime-Not-Applicable）。其余 22 个示例和全部 16 个实验共 38 个主体待硬件验证（Pending Hardware Verification）。P01-P12 和 T01-T05 四个证据数组为空。EX21-EX23 编译/已记录观察为空，主机检查不授予 GPU 证据。运行已验证（Runtime-Verified）、社区已观察（Community-Observed）、基准环境（Reference Environment）、性能观察均为零。六份分析器计划只有预期内容。
