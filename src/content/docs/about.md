...
截至 **2026-09-14**，当前发布有 90 个学习单元、23 个可运行示例（EX01-EX23）、15 个实验（LAB01-LAB15）、20 项可视化讲解、98 个[练习题库（Practice Bank）](/practice/)条目、207 个[术语表](/glossary/)词条、104 条[来源记录](/sources-and-versions/)，共 444 条资源目录记录、329 个双语发布对、658 条源路由，含 89 组练习与 89 组独立参考解答。[T01](/triton/programs-and-block-values/) 与 [T02](/triton/masked-vector-addition/) 提供 Triton 心智模型与向量加法，并配有 [EX23](/examples/triton-vector-add/) 和 [VIS17](/visuals/simt-triton-mapping/)；[T03](/triton/fused-softmax/) 与 [LAB15](/labs/verify-fused-softmax/) 新增融合 Softmax。
...
当前 98 道练习题库题目包括 PB-R5-001 至 PB-R5-016。R4 八道库与算法选择题、R3 十道 Nsight 报告分析题不变。只有 EX02、EX10、LAB02 编译已检查（Compile-Checked），EX10 无需运行验证（Runtime-Not-Applicable）。其余 22 个示例和全部 15 个实验共 37 个主体待硬件验证（Pending Hardware Verification）。P01-P12 和 T01-T03 四个证据数组为空。EX21-EX23 编译/已记录观察为空，主机检查不授予 GPU 证据。运行已验证（Runtime-Verified）、社区已观察（Community-Observed）、基准环境（Reference Environment）、性能观察均为零。六份分析器计划只有预期内容。
