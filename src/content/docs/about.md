...
当前 115 道练习题库题目包括 PB-R5-001 至 PB-R5-020 以及 PB-R6-001 至 PB-R6-013。R4 八道库与算法选择题、R3 十道 Nsight 报告分析题不变。只有 EX02、EX10、LAB02 编译已检查（Compile-Checked），EX10 无需运行验证（Runtime-Not-Applicable）。其余 23 个示例和全部 18 个实验共 41 个主体待硬件验证（Pending Hardware Verification）。P01-P12、T01-T08 和 G01-G09 四个证据数组为空。EX21-EX24 编译/已记录观察为空，主机检查不授予 GPU 证据。运行已验证（Runtime-Verified）、社区已观察（Community-Observed）、基准环境（Reference Environment）、性能观察均为零。六份分析器计划只有预期内容，LAB18 时间线及 G07／G09 诊断材料为合成数据。
