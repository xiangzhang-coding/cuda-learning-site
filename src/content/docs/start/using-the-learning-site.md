...
截至 **2026-09-02**，[issue #27](https://github.com/xiangzhang-coding/cuda-learning-site/issues/27) 后的稳定课程当前滚动发布已完整双语发布 56 个学习单元：O01-O08、F01-F08、M01-M19、A01-A09/A14，以及 Q01-Q11。下面是当前完整发布的严格先修关系：
...
13. 正确性与质量路线保留 Q01-Q08，并加入 [Q09：用 Occupancy、Stalls 与 Throughput 解释延迟隐藏](/correctness/occupancy-stalls-throughput/)`<-[Q08,F08]`、[Q10：从 Arithmetic Intensity 构建可审计 Roofline](/correctness/roofline-arithmetic-intensity/)`<-[Q05,A14]`与 [Q11：用受控证据优化 Canonical Transpose](/correctness/transpose-optimization-case-study/)`<-[A05,Q06,Q08,Q10]`。Q11 从 immutable [EX14](/examples/tiled-transpose/)开始，并复用 evidence-neutral [VIS11](/visuals/tiled-transpose/)。
14. Runnable Example 集合仍是 EX01-EX16。Lab 集合是 LAB01-LAB10，其中 [LAB09：构建原创 Roofline](/labs/build-original-roofline/)`<-[Q10]`，[LAB10：优化 canonical transpose](/labs/optimize-canonical-transpose/)`<-[Q11]`；Visual graph 仍包含 [VIS13：Roofline](/visuals/roofline/)`<-[Q10]`。Q13、L06 与 LAB12 未发布，LAB12 必须等待 Q13 与 L06。
...
完成这些基础后，算法路线在 A09 后加入由 A01/A02/A05/A08 汇入的 A14；正确性路线在 Q08 后加入由 Q08/F08 汇入的 Q09，再由 Q05/A14 汇入 Q10，最后由 A05/Q06/Q08/Q10 汇入 Q11。LAB09 与 VIS13 严格依赖 Q10，LAB10 严格依赖 Q11。Q13、L06 与 LAB12 仍在当前依赖图之外，LAB12 必须等待 Q13 与 L06。
...
Q06-Q11 都有直接练习与独立参考解答，包括 [Q09 练习](/correctness/occupancy-stalls-throughput/exercises/)与[解答](/correctness/occupancy-stalls-throughput/solutions/)、[Q10 练习](/correctness/roofline-arithmetic-intensity/exercises/)与[解答](/correctness/roofline-arithmetic-intensity/solutions/)，以及 [Q11 练习](/correctness/transpose-optimization-case-study/exercises/)与[解答](/correctness/transpose-optimization-case-study/solutions/)。
...
[实验索引](/labs/)列出 10 个 Labs（LAB01-LAB10），包括 [LAB10](/labs/optimize-canonical-transpose/)。[可视化讲解索引](/visuals/)仍列出 18 项讲解：独立 VIS01-VIS14，加上内嵌 VIS19-VIS22。当前 catalog 是 10 个 Lab、58 个[练习题库](/practice/)条目、18 项 Visual Explainer、165 项[术语表](/glossary/)和 70 项[来源记录](/sources-and-versions/)，共 321 条记录；当前滚动公开内容形成 213 个 Publication Pairs 和 426 条 source routes。这些数字独立于不可变的 R2 186/372/284 快照，R3 聚合复核仍待完成。

Q11 作为 Learning Unit，其 compilation、runtime、expected-observation 与 recorded-observation 四个 evidence arrays 均为空；它不授予 Evidence Status，只汇总关联对象。关联的 EX14 与 LAB10 的 compilation 和 recorded observations 为空、runtime 为 Pending Hardware Verification；immutable EX14 source、VIS11、static/browser evidence 与 expected-only fixture 都不会改变这些边界。LAB10 声明 `4096x4096` `float` workload、134,221,952-byte 保守上界、一次排除的 warm-up、显式同步，以及十次 attempts 的 median 与 min/max 方法，但没有 recorded result。本次增量没有 timing、metric、speedup、bottleneck、winner、Reference Environment 或 `performanceObservations`。
...
**事实核查日期：2026-09-02。** 本页不依赖特定 CUDA Toolkit 版本，也不会授予 CUDA 证据状态。术语定义见[术语表](/glossary/)，发布接口与 CUDA 来源依据见[来源与版本记录](/sources-and-versions/)。
