...
[实验索引](/labs/)列出 12 个实验（LAB01-LAB12）。[可视化讲解索引](/visuals/)列出 19 项讲解：独立 VIS01-VIS14/VIS18，加上内嵌 VIS19-VIS22。当前目录有 89 个[练习题库](/practice/)条目、204 个[术语表](/glossary/)词条和 99 条[来源记录](/sources-and-versions/)，共 423 条资源目录记录；内容形成 299 个双语发布对和 598 条源路由。在 Python 桥接题 `PB-R5-001/002/003` 之后新增 PyTorch 题 `PB-R5-004/005/006/007`；R3 十道 Nsight 报告分析题和 R4 八道库与算法选择题保持不变：`PB-R4-001`、`PB-R4-002`、`PB-R4-003`、`PB-R4-004`、`PB-R4-008`、`PB-R4-011`、`PB-R4-012` 和 `PB-R4-016`。

P04-P07 四个证据数组保持为空。[独立 eager 环境](/sources-and-versions/#src-cuda-080)选择 PyTorch 2.11.0+cu128、CPython 3.12.14、CUDA 元包 12.8.1、runtime/CUPTI 12.8.90、cuDNN 9.19.0.56 及 native 分配器。不需要系统 Toolkit 或 nvcc，也不继承 EX21 配置或已有工具包通道（Toolkit Lane）证据。源码检查不是 GPU 测量。P08、EX22、LAB13 与 Triton 目的地仍未发布。
