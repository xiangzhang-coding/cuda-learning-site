...
10. 正确性与质量路线使用非连续编号：[Q01：CPU 参考实现、容差与不变量](/correctness/cpu-references-tolerances-invariants/)同时依赖 F04 和 O04；[Q03：用 memcheck 定位非法内存访问](/correctness/memcheck-invalid-memory-access/)同时依赖 F05 和 Q01；[Q04：用 racecheck、initcheck 与 synccheck 定位缺陷](/correctness/racecheck-initcheck-synccheck/)同时依赖 M05、M06 和 Q03；[Q05：诚实计时异步 GPU 工作](/correctness/timing-asynchronous-gpu-work/)同时依赖 M08 和 Q01。Q02 没有公开页面或导航占位。
11. [EX03 多维索引可运行示例](/examples/multidimensional-indexing/)依赖 F03，并为 F03/F04 提供 canonical source；[EX04 错误处理生命周期可运行示例](/examples/error-handling-lifecycle/)仅依赖 F05；[EX05 合并与跨步访问可运行示例](/examples/coalesced-strided-access/)仅依赖 M02；[EX06 共享内存 tile bank padding 可运行示例](/examples/shared-memory-tile-bank-padding/)同时依赖 M03 和 M04；[EX16 Compute Sanitizer 缺陷套件](/examples/sanitizer-defect-suite/)同时依赖 Q03 和 Q04。七个非连续 Runnable Example 严格是 EX01-EX06 与 EX16；EX07-EX15 不存在公开目标。[LAB02](/labs/vector-addition/)同时依赖 O03 和 F01，并使用 canonical [EX02](/examples/vector-addition/)；[LAB03：破坏并修复索引](/labs/break-and-repair-indexing/)同时依赖 F03 和 F05，并使用 EX04；[LAB04：观察合并访问](/labs/observe-coalescing/)同时依赖 M02 和 Q05，并使用 EX05；[LAB05：消除共享内存存储体冲突](/labs/remove-shared-memory-bank-conflicts/)同时依赖 M04 和 Q05，并使用 EX06；[LAB07：诊断四类 Sanitizer 故障](/labs/diagnose-four-sanitizer-failures/)同时依赖 Q03 和 Q04，并使用 EX16。六个非连续 Lab 严格是 LAB01-LAB05 与 LAB07；LAB06 不存在公开目标。F08 与 LAB03 相关，但不是 LAB03 的先修条件。

下一步应按自己的缺口选择。O04 的 C++17 复习可在阅读 F01-F08 和所有已发布 Runnable Example 时使用，但不会成为它们的新先修条件。Linux 记录路线必须合并 O02、O03 和 O05 后才进入 O08；EX01 可直接查阅，LAB01 则必须同时满足 O03 和 O08。架构路线按 O06、O07 的顺序学习，再把这些模型用于基础课程。F05 从 F04 继续错误生命周期；F06 汇合 F02/O03；F07 汇合 F04/F05；F08 汇合 F02/F03/F06。内存路线从 F04/F06 汇入 M01，经 M02-M04 建立 memory access/layout，再分别进入 M05/M06 的同步与 warp 推理，以及由 F05/M01 汇入 M07、再到 M08 的 stream/event 路线。正确性路线从 F04/O04 汇入 Q01，随后分别进入 F05/Q01 汇成的 Q03、M05/M06/Q03 汇成的 Q04，以及 M08/Q01 汇成的 Q05；Q05 再为 LAB04/LAB05 提供测量合同，Q03/Q04 则通向 EX16/LAB07。这些学习建议不增加上面没有列出的先修边。

O02-O08、F01-F08、M01-M08 与 Q01/Q03-Q05 都有直接练习和独立参考解答。正确性路线可直接进入 [Q01 练习](/correctness/cpu-references-tolerances-invariants/exercises/)与[解答](/correctness/cpu-references-tolerances-invariants/solutions/)、[Q03 练习](/correctness/memcheck-invalid-memory-access/exercises/)与[解答](/correctness/memcheck-invalid-memory-access/solutions/)、[Q04 练习](/correctness/racecheck-initcheck-synccheck/exercises/)与[解答](/correctness/racecheck-initcheck-synccheck/solutions/)、[Q05 练习](/correctness/timing-asynchronous-gpu-work/exercises/)与[解答](/correctness/timing-asynchronous-gpu-work/solutions/)；既有单元仍可通过上方严格先修图直接访问。

[实验索引](/labs/)按当前顺序直接列出 [LAB01](/labs/record-cuda-environment/)、[LAB02](/labs/vector-addition/)、[LAB03](/labs/break-and-repair-indexing/)、[LAB04](/labs/observe-coalescing/)、[LAB05](/labs/remove-shared-memory-bank-conflicts/)和 [LAB07](/labs/diagnose-four-sanitizer-failures/)；这些是六个完整公开实验，LAB06 不在导航中。[可视化讲解索引](/visuals/)列出十一项正式讲解：七个独立页面 [VIS01](/visuals/kernel-journey/)、[VIS02](/visuals/indexing/)、[VIS03](/visuals/warp-divergence/)、[VIS04](/visuals/memory-transactions/)、[VIS05](/visuals/shared-memory-banks/)、[VIS06](/visuals/memory-hierarchy-lifetime/)与 [VIS07](/visuals/stream-event-dependencies/)，以及内嵌 VIS19-VIS22。十一项讲解都是确定性的 browser-only model，提供静态或文字回退，不执行 CUDA，也没有 CUDA Evidence Status。最终 catalog 是 6 个 Lab、29 个[练习题库](/practice/)条目、11 项 Visual Explainer、95 项[术语表](/glossary/)和 39 项[来源与版本记录](/sources-and-versions/)，共 180 条记录；公开内容形成 109 个双语发布对（Publication Pair）和 218 条 source route。你还可以直接查阅[关于本站](/about/)。

Q01/Q03-Q05 的 compilation/runtime axes 均为空；静态学习材料、练习和 Context7 cross-check 不授予 CUDA Evidence Status。EX01 保留现有状态：没有 Compile-Checked 声明，runtime 为 Pending Hardware Verification。EX03-EX06 各自在 11.8.0、12.9.2 与 13.3.1 三条工具包通道（Toolkit Lane）使用一份原创 C++17 实现；EX16 是一个跨相同 Lanes 构建八个隔离 binary 的原创 Apache-2.0 C++17 project。EX05、EX06、EX16、LAB04、LAB05 与 LAB07 的 compilation evidence 为空，runtime 为 Pending Hardware Verification，recorded observations 为空，只提供 expected observations。Host-only checks 不编译或运行 CUDA，也不能建立 GPU correctness 或 sanitizer behavior；本站没有运行 EX03-EX06/EX16 CUDA binary、Compute Sanitizer 或 profiler，也没有记录实际 output、sanitizer report、timing、speedup 或其他 performance number。
...
**事实核查日期：2026-08-28。** 本页不依赖特定 CUDA Toolkit 版本，也不会授予 CUDA 证据状态。术语定义见[术语表](/glossary/)，发布接口与 CUDA 来源依据见[来源与版本记录](/sources-and-versions/)。
