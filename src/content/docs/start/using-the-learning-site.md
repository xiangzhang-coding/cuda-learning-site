...
截至 **2026-09-05**，[issue #32](https://github.com/xiangzhang-coding/cuda-learning-site/issues/32) 完成复核的 R3 稳定课程已完整双语发布 62 个学习单元：O01-O08、F01-F08、M01-M19、A01-A14，以及 Q01-Q13。当前滚动发布通过 issue #33 新增 L01-L02，通过 [issue #34](https://github.com/xiangzhang-coding/cuda-learning-site/issues/34)发布 [L03](/libraries/cub-device-primitives/)、[L04](/libraries/cub-warp-block-primitives/)、[EX17](/examples/cub-device-reduction-scan/)与 [LAB11](/labs/compare-custom-reduction-with-cub/)，随后发布 [L05：libcu++ 同步抽象](/libraries/libcu-plus-plus-synchronization/)。[Issue #36](https://github.com/xiangzhang-coding/cuda-learning-site/issues/36)再新增 [L06：cuBLAS GEMM](/libraries/cublas-gemm/)、[L07：cuBLASLt Matmul](/libraries/cublaslt-matmul/)、[EX18](/examples/cublas-gemm/)与 [LAB12](/labs/compare-gemm-with-cublas/)；[issue #37](https://github.com/xiangzhang-coding/cuda-learning-site/issues/37)新增 [L08：张量核心（Tensor Core）精度与架构合同](/libraries/tensor-core-precision-contracts/)和 [L09：CUTLASS C++ GEMM 结构](/libraries/cutlass-cpp-gemm-structure/)，当前共有 71 个学习单元。下面列出当前完整发布的严格先修关系。

已完成的 R3 聚合发布复核是不可变快照，固定为 232 个 Publication Pairs、464 条 source routes 与 347 条 catalog records；它还明确清点 61 组 Exercises、61 组独立 reviewed solutions 和 10 个 Nsight report-analysis Practice Bank 条目。当前滚动发布有 263 个双语发布对、526 条源路由、70 组练习、70 组独立参考解答与 383 条资源目录记录。R1/R2 继续作为历史坐标，R4 聚合复核仍待完成。

**发布清单与导航更新：2026-09-07。** 页面日期仅对应本次发布清单更新，不重新核查既有 CUDA API 事实；技术来源及其访问、复核日期不变。
...
16. 可运行示例集合是 EX01-EX18，最新项目为 [EX18：cuBLAS GEMM](/examples/cublas-gemm/)`<-[L06]`；实验集合是 LAB01-LAB12，最新活动为 [LAB12：对比教学 GEMM 与 cuBLAS](/labs/compare-gemm-with-cublas/)`<-[Q13,L06]`。可视化路线保留 [VIS18：Attention Memory Traffic](/visuals/attention-memory-traffic/)`<-[A11]`；L10-L13 与 EX20 仍待发布。
...
**发布清单核对日期：2026-09-07。** 本页不依赖特定 CUDA Toolkit 版本，也不会授予 CUDA 证据状态。术语定义见[术语表](/glossary/)，发布接口与 CUDA 来源依据见[来源与版本记录](/sources-and-versions/)。
