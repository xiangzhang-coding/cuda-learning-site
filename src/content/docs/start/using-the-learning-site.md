...
最近完成的聚合静态复核仍是 2026-09-10 的 R4。冻结的 `/release.json` 保留 75 个学习单元、277 个双语发布对、554 条源路由、74 组练习、74 组独立解答、401 条目录记录。R1-R3 是不变的历史快照。`/publication.json` 当前记录 317 个双语发布对、634 条源路由、86 组练习和 86 组独立参考解答。R5 聚合复核仍待完成，发布不升级运行证据。
...
当前 87 个学习单元扩展至 P12。[P11](/frameworks/profile-led-optimization/)依赖 `[P07,P09,Q06]`；[P12](/frameworks/sdpa-dispatch-verification/)依赖 `[A11,L11,P06,P07]`。[实验索引](/labs/)列出 14 个实验，[可视化讲解索引](/visuals/)列出 19 项讲解。有 94 个[练习题库](/practice/)条目、204 个[术语表](/glossary/)词条、102 条[来源记录](/sources-and-versions/)，共 433 条资源目录记录。PB-R5-011/012 增加原创优化/派发审查题；R3 十道 Nsight 报告分析题和 R4 八道库与算法选择题不变。
...
只有 EX02、EX10、LAB02 编译已检查（Compile-Checked）。EX10 无需运行验证（Runtime-Not-Applicable）；其余 21 个示例和 14 个实验共 35 个主体待硬件验证（Pending Hardware Verification）。P01-P12 证据数组为空。EX21/EX22 与 LAB13/LAB14 编译/已记录观察为空，仍待硬件验证。运行已验证（Runtime-Verified）、社区已观察（Community-Observed）、基准环境（Reference Environment）、性能观察均为零；来源和主机检查不是 GPU 证据。
