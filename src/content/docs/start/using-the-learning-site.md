...
10. [EX03 多维索引可运行示例](/examples/multidimensional-indexing/)依赖 F03，并为 F03/F04 提供 canonical source；[EX04 错误处理生命周期可运行示例](/examples/error-handling-lifecycle/)仅依赖 F05；[EX05 合并与跨步访问可运行示例](/examples/coalesced-strided-access/)仅依赖 M02；[EX06 共享内存 tile bank padding 可运行示例](/examples/shared-memory-tile-bank-padding/)同时依赖 M03 和 M04。Runnable Example 严格只有 EX01-EX06，没有 EX07。[LAB02](/labs/vector-addition/)同时依赖 O03 和 F01，并使用 canonical [EX02](/examples/vector-addition/)；[LAB03：破坏并修复索引](/labs/break-and-repair-indexing/)同时依赖 F03 和 F05，并使用 EX04。F08 与 LAB03 相关，但不是 LAB03 的先修条件；Lab 严格只有 LAB01-LAB03。

下一步应按自己的缺口选择。O04 的 C++17 复习可在阅读 F01-F08 和 EX02-EX06 时使用，但不会成为它们的新先修条件。Linux 记录路线必须合并 O02、O03 和 O05 后才进入 O08；EX01 可直接查阅，LAB01 则必须同时满足 O03 和 O08。架构路线按 O06、O07 的顺序学习，再把这些模型用于基础课程。F05 从 F04 继续错误生命周期；F06 汇合 F02/O03；F07 汇合 F04/F05；F08 汇合 F02/F03/F06。内存路线从 F04/F06 汇入 M01，经 M02-M04 建立 memory access/layout，再分别进入 M05/M06 的同步与 warp 推理，以及由 F05/M01 汇入 M07、再到 M08 的 stream/event 路线。这些学习建议不增加上面没有列出的先修边。

O02-O08、F01-F08 与 M01-M08 都有直接练习和独立参考解答。新增内存路线可直接进入 [M05 练习](/memory/synchronization-scopes/exercises/)与[解答](/memory/synchronization-scopes/solutions/)、[M06 练习](/memory/warp-divergence-reconvergence/exercises/)与[解答](/memory/warp-divergence-reconvergence/solutions/)、[M07 练习](/memory/stream-ordering/exercises/)与[解答](/memory/stream-ordering/solutions/)、[M08 练习](/memory/event-dependencies-timing/exercises/)与[解答](/memory/event-dependencies-timing/solutions/)；既有早期单元仍可通过上方严格先修图直接访问。

[实验索引](/labs/)按当前顺序直接列出 [LAB01](/labs/record-cuda-environment/)、[LAB02](/labs/vector-addition/)和 [LAB03](/labs/break-and-repair-indexing/)；公开实验严格只有这三项，导航没有未完成学习单元。[可视化讲解索引](/visuals/)列出十一项正式讲解：七个独立页面 [VIS01](/visuals/kernel-journey/)、[VIS02](/visuals/indexing/)、[VIS03](/visuals/warp-divergence/)、[VIS04](/visuals/memory-transactions/)、[VIS05](/visuals/shared-memory-banks/)、[VIS06](/visuals/memory-hierarchy-lifetime/)与 [VIS07](/visuals/stream-event-dependencies/)，以及内嵌 VIS19-VIS22。十一项讲解都是确定性的 browser-only model，提供静态或文字回退，不执行 CUDA，也没有 CUDA Evidence Status。最终 catalog 是 3 个 Lab、25 个[练习题库](/practice/)条目、11 项 Visual Explainer、86 项[术语表](/glossary/)和 36 项[来源与版本记录](/sources-and-versions/)，共 161 项；公开内容形成 93 个双语发布对（Publication Pair）和 186 条 source route。你还可以直接查阅[关于本站](/about/)。
