...
截至 **2026-09-05**，[issue #32](https://github.com/xiangzhang-coding/cuda-learning-site/issues/32) 完成复核的 R3 稳定课程已完整双语发布 62 个学习单元：O01-O08、F01-F08、M01-M19、A01-A14，以及 Q01-Q13。Rolling current surface 通过 issue #33 新增 L01-L02，并通过 [issue #34](https://github.com/xiangzhang-coding/cuda-learning-site/issues/34)发布 [L03](/libraries/cub-device-primitives/)、[L04](/libraries/cub-warp-block-primitives/)、[EX17](/examples/cub-device-reduction-scan/)与 [LAB11](/labs/compare-custom-reduction-with-cub/)，再新增 [L05：libcu++ 同步抽象](/libraries/libcu-plus-plus-synchronization/)，当前共有 67 个 Learning Units。下面是当前完整发布的严格先修关系：

已完成的 R3 聚合发布复核是不可变快照，固定为 232 个 Publication Pairs、464 条 source routes 与 347 条 catalog records；它还明确清点 61 组 Exercises、61 组独立 reviewed solutions 和 10 个 Nsight report-analysis Practice Bank 条目。Rolling current surface 有 249 个 Publication Pairs、498 条 source routes、66 组 Exercises、66 组独立 reviewed solutions 与 370 条 catalog records。R1/R2 继续作为历史坐标，R4 聚合复核仍待完成。
...
12. 并行算法路线保留 A01-A11，加入 [A12：稀疏格式、存储账本与 SpMV](/algorithms/sparse-formats-spmv/)`<-[M01,M02]`、[A13：稀疏矩阵乘法、预处理与工作区决策](/algorithms/sparse-matrix-multiplication-preprocessing/)`<-[A12,A08]`，并保留 [A14](/algorithms/algorithm-choice-arithmetic-intensity/)`<-[A01,A02,A05,A08]`。
...
14. 可复用库路线包含 [L01：在生产库、可复用原语、DSL 与自定义内核之间做选择](/libraries/library-primitive-dsl-custom-kernel/)`<-[A02,A03,A08,Q06]`、[L02：把 Thrust 作为算法词汇](/libraries/thrust-algorithm-vocabulary/)`<-[A01,A03,A09]`、[L03：用 CUB Device Primitives 完成归约与扫描](/libraries/cub-device-primitives/)`<-[A02,A03,M07,L01]`、[L04：用 CUB Warp 与 Block Primitives 组合内核](/libraries/cub-warp-block-primitives/)`<-[F02,M03,M05,A02,A03,L03]`和 [L05：libcu++ 同步抽象](/libraries/libcu-plus-plus-synchronization/)`<-[M05,M13,M19]`。
15. Runnable Example 集合是 EX01-EX17，最新项目为 [EX17：CUB Device Reduction and Scan](/examples/cub-device-reduction-scan/)；Lab 集合是 LAB01-LAB11，最新活动为 [LAB11：比较自定义归约与 CUB](/labs/compare-custom-reduction-with-cub/)`<-[Q12,L03]`。Visual graph 保留 [VIS18：Attention Memory Traffic](/visuals/attention-memory-traffic/)`<-[A11]`；L06-L13、LAB12 与 EX20 仍待发布。
...
完成这些基础后，算法路线从 A02/M02/M03 汇入 A10，再由 A08/A10 汇入 A11；M01/M02 汇入 A12，A12/A08 再汇入 A13；A14 继续由 A01/A02/A05/A08 汇入。A02/A03/A08/Q06 汇入 L01，A01/A03/A09 汇入 L02，A02/A03/M07/L01 汇入 L03，F02/M03/M05/A02/A03/L03 汇入 L04，M05/M13/M19 汇入 L05；Q12/L03 汇入 LAB11，EX17 提供其 canonical CUB path。VIS18 严格依赖 A11。L06-L13、LAB12 与 EX20 不进入当前图。
...
可复用库路线可进入 [L01 练习](/libraries/library-primitive-dsl-custom-kernel/exercises/)与[解答](/libraries/library-primitive-dsl-custom-kernel/solutions/)、[L02 练习](/libraries/thrust-algorithm-vocabulary/exercises/)与[解答](/libraries/thrust-algorithm-vocabulary/solutions/)、[L03 练习](/libraries/cub-device-primitives/exercises/)与[解答](/libraries/cub-device-primitives/solutions/)、[L04 练习](/libraries/cub-warp-block-primitives/exercises/)与[解答](/libraries/cub-warp-block-primitives/solutions/)，以及 [L05 练习](/libraries/libcu-plus-plus-synchronization/exercises/)与[解答](/libraries/libcu-plus-plus-synchronization/solutions/)。

[实验索引](/labs/)列出 11 个 Labs（LAB01-LAB11）。[可视化讲解索引](/visuals/)列出 19 项讲解：独立 VIS01-VIS14/VIS18，加上内嵌 VIS19-VIS22。Immutable R3 保留 66 个 Practice Bank 条目、176 个 Glossary terms、76 条 source records 与 347 条 catalog records。Rolling current catalog 有 72 个[练习题库](/practice/)条目、186 项[术语表](/glossary/)和 82 项[来源记录](/sources-and-versions/)，共 370 条记录；current content 形成 249 个 Publication Pairs 和 498 条 source routes。R4 聚合复核仍待完成。

Q06-Q13、A10-A14 与 L01-L05 的四个 evidence arrays 均为空且不授予 Evidence Status。EX17 与 LAB11 的 compilation evidence 和 recorded observations 为空、没有 observed runtime evidence，并保持待硬件验证（Pending Hardware Verification）。EX17 的五项 component matrix 是 non-evidentiary build gate，不表示 CI 已运行。当前六份 profiler fixtures 都是 expected-only plans，不是 captured reports；current publication 不声明 Reference Environment 或 `performanceObservations`。
...
**事实核查日期：2026-09-05。** 本页不依赖特定 CUDA Toolkit 版本，也不会授予 CUDA 证据状态。术语定义见[术语表](/glossary/)，发布接口与 CUDA 来源依据见[来源与版本记录](/sources-and-versions/)。
