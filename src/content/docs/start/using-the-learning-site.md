...
最近完成的聚合静态复核仍是 2026-09-10 的 R4。冻结的 `/release.json` 保留 75 个学习单元、277 个双语发布对、554 条源路由、74 组练习、74 组独立解答、401 条目录记录。R1-R3 是不变的历史快照。`/publication.json` 当前记录 325 个双语发布对、650 条源路由、88 组练习和 88 组独立参考解答。R5 聚合复核仍待完成，发布不升级运行证据。
...
16. 23 个可运行示例为 EX01-EX23；[EX21](/examples/cuda-python-launch/)保留 `[P02]`，[EX20](/examples/cusparse-spmv/)保留 `[L13]`，[EX19](/examples/cufft-batched-transform/)保留 `[L12]`，[EX18](/examples/cublas-gemm/)保留 `[L06]`。14 个实验为 LAB01-LAB14，[LAB12](/labs/compare-gemm-with-cublas/)保留 `[Q13,L06]`。[EX22](/examples/adjacent-energy/)、[LAB13](/labs/build-custom-operator/)均依赖 `[P08,P09]`。[LAB14](/labs/profile-custom-operator/)依赖 `[P11]`。[VIS18](/visuals/attention-memory-traffic/)仍依赖 `[A11]`。[EX23](/examples/triton-vector-add/) 依赖 `[T02]`，[VIS17](/visuals/simt-triton-mapping/) 依赖 `[T01]`。
...
当前 89 个学习单元扩展至 P12 和 T02。[T01](/triton/programs-and-block-values/) 依赖 `[F02,F03,M02]`；[T02](/triton/masked-vector-addition/) 依赖 `[T01,A01]`。[实验索引](/labs/)列出 14 个实验，[可视化讲解索引](/visuals/)列出 20 项讲解。有 96 个[练习题库](/practice/)条目、207 个[术语表](/glossary/)词条、103 条[来源记录](/sources-and-versions/)，共 440 条资源目录记录。PB-R5-013/014 增加原创 Triton 布局与正确性审查题；R3 十道 Nsight 报告分析题和 R4 八道库与算法选择题不变。

P04-P07 的证据数组为空，[独立 eager 环境](/sources-and-versions/#src-cuda-080)保留 torch 2.11.0+cu128、CPython 3.12.14、随包 CUDA 12.8.1/runtime 12.8.90 与 native 分配器。Eager 工作不需要系统 Toolkit。[P08](/frameworks/first-custom-operator/)依赖 `[O04,F04,Q01,P04]`，[P09](/frameworks/operator-registration/)依赖 `[P08,Q01]`，[P10](/frameworks/operator-packaging/)依赖 `[P08,M18]`，均含练习和独立解答。扩展项目另行要求 Toolkit 12.8.1/NVCC 12.8.93/GCC 13.3.0。T01/T02 与 EX23 使用独立 Triton 3.7.1 环境；后续 Triton 单元尚未发布。

只有 EX02、EX10、LAB02 编译已检查（Compile-Checked）。EX10 无需运行验证（Runtime-Not-Applicable）；其余 22 个示例和 14 个实验共 36 个主体待硬件验证（Pending Hardware Verification）。P01-P12 和 T01-T02 证据数组为空。EX21-EX23 与 LAB13/LAB14 编译/已记录观察为空，仍待硬件验证。运行已验证（Runtime-Verified）、社区已观察（Community-Observed）、基准环境（Reference Environment）、性能观察均为零；来源和主机检查不是 GPU 证据。
