...
最近完成的聚合静态复核仍是 2026-09-10 的 R4。冻结的 `/release.json` 保留 75 个学习单元、277 个双语发布对、554 条源路由、74 组练习、74 组独立解答、401 条目录记录。R1-R3 是不变的历史快照。`/publication.json` 当前记录 336 个双语发布对、672 条源路由、91 组练习和 91 组独立参考解答。R5 聚合复核仍待完成，发布不升级运行证据。
...
16. 23 个可运行示例为 EX01-EX23；[EX21](/examples/cuda-python-launch/)保留 `[P02]`，[EX20](/examples/cusparse-spmv/)保留 `[L13]`，[EX19](/examples/cufft-batched-transform/)保留 `[L12]`，[EX18](/examples/cublas-gemm/)保留 `[L06]`。16 个实验为 LAB01-LAB16，[LAB12](/labs/compare-gemm-with-cublas/)保留 `[Q13,L06]`。[EX22](/examples/adjacent-energy/)、[LAB13](/labs/build-custom-operator/)均依赖 `[P08,P09]`。[LAB14](/labs/profile-custom-operator/)依赖 `[P11]`，[LAB15](/labs/verify-fused-softmax/)依赖 `[T02,A10,Q05]`，[LAB16](/labs/autotune-triton-gemm/)依赖 `[T04,T05]`。[VIS18](/visuals/attention-memory-traffic/)仍依赖 `[A11]`。[EX23](/examples/triton-vector-add/) 依赖 `[T02]`，[VIS17](/visuals/simt-triton-mapping/) 依赖 `[T01]`。
...
21. [P04：排队工作与计时](/frameworks/queued-work-timing/)先修 `[M07,Q05]`；[P05：流与存储生命周期](/frameworks/streams-and-storage-lifetime/)先修 `[P04,M08]`；[P06：混合精度合同](/frameworks/mixed-precision-contracts/)先修 `[Q02,P04,L08]`；[P07：从 Python 到 CUDA 的性能分析](/frameworks/python-to-cuda-profiling/)先修 `[P04,Q07,Q08]`。相关边依次为 `[P05,P06,P07]`、`[P04]`、`[P04,P07]`、`[P05,P06]`。各练习集只依赖本单元，独立解答只依赖对应练习集。Issue #43 新增这十二对，不新增可运行示例或实验。
...
完成这些基础后，算法路线从 A02/M02/M03 汇入 A10，再由 A08/A10 汇入 A11；M01/M02 汇入 A12，A12/A08 再汇入 A13；A14 继续由 A01/A02/A05/A08 汇入。A02/A03/A08/Q06 汇入 L01，A01/A03/A09 汇入 L02，A02/A03/M07/L01 汇入 L03，F02/M03/M05/A02/A03/L03 汇入 L04，M05/M13/M19 汇入 L05，A08/Q01 汇入 L06，L06/Q05 汇入 L07，Q02/L06/F06 汇入 L08，A08/L06/M17 汇入 L09，A07/L01/Q05 汇入 L10，A11/L10/L08 汇入 L11，Q05/M07 汇入 L12，A12/A13/L01 汇入 L13。Q12/L03 汇入 LAB11，EX17 提供其规范 CUB 实现；Q13/L06 汇入 LAB12，由 EX15 与 EX18 提供对比实现。VIS18 严格依赖 A11，EX20 严格依赖 L13。
...
当前 92 个学习单元扩展至 P12 和 T05。[T04](/triton/blocked-matrix-multiplication/) 依赖 `[T02,A08,Q10]`；[T05](/triton/autotuning/) 依赖 `[T04,Q05,Q06]`。[实验索引](/labs/)列出 16 个实验，[可视化讲解索引](/visuals/)列出 20 项讲解。有 100 个[练习题库](/practice/)条目、207 个[术语表](/glossary/)词条、105 条[来源记录](/sources-and-versions/)，共 448 条资源目录记录。PB-R5-017/018 增加原创 GEMM 尾部与选择缓存审查题；R3 十道 Nsight 报告分析题和 R4 八道库与算法选择题不变。
...
只有 EX02、EX10、LAB02 编译已检查（Compile-Checked）。EX10 无需运行验证（Runtime-Not-Applicable）；其余 22 个示例和 16 个实验共 38 个主体待硬件验证（Pending Hardware Verification）。P01-P12 和 T01-T05 证据数组为空。EX21-EX23 与 LAB13-LAB16 编译/已记录观察为空，仍待硬件验证。运行已验证（Runtime-Verified）、社区已观察（Community-Observed）、基准环境（Reference Environment）、性能观察均为零；来源和主机检查不是 GPU 证据。
...
L08 区分输入转换、累加、输出转换与 WMMA 架构合同；L09 以固定的 CUTLASS C++ v4.7.0、提交 `dcf215af68a2d08d305076c152a06f201728cd53` 阅读 GEMM 结构，不承诺 DSL 或可执行支持。两者都没有本地编译、真实指令观察、CUDA 运行或性能结果。VIS12 的模型与标量 instruction panel 不变，不能把它读成真实 Tensor Core 指令。

[L10 练习](/libraries/cudnn-graphs-and-plans/exercises/)与[独立解答](/libraries/cudnn-graphs-and-plans/solutions/)、[L11 练习](/libraries/attention-backend-dispatch/exercises/)与[独立解答](/libraries/attention-backend-dispatch/solutions/)进一步检查图/计划和分派边界。两单元独立固定 cuDNN backend 9.24.0 与 frontend 1.27.0、提交 `f77fbc3d21be3f24cd0286b9b368105f7c518b8a`，只作参考阅读，不复制上游代码/资产，不建立编译、GPU、实际引擎、类型或性能证据。

L12 使用归档的 12.9.2 cuFFT 教学基线和访问于 2026-09-08 的当前 13.3 参考；精确的 13.3.1 API 与发布说明归档不可用。EX19 声明同样三条固定工具包通道（Toolkit Lane），仅使用 C++17，分别搭配捆绑的 cuFFT 10.9.0.58 / 11.4.1.4 / 12.3.0.29。其 FP32 C2C 合同不包含回调（callback）、低精度、多 GPU 执行或计时；不可变源码和构建检查不形成 GPU 证据。

L13 使用归档的 12.9.2 cuSPARSE 教学基线；[SRC-CUDA-075/076 复核](/sources-and-versions/#src-cuda-075)于 2026-09-09 记录了可访问的精确归档，包括 13.3.1，与上述 cuFFT 历史访问记录分开。EX20 的三项 C++17 非执行构建门槛固定 Toolkit/cuSPARSE 组合为 11.8.0/11.7.5.86、12.9.2/12.5.10.65 与 13.3.1/12.8.2.51；来源审查与构建门槛不是 GPU 证据。
