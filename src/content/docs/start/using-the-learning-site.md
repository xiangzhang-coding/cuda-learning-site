...
截至 **2026-08-29**，稳定课程已完整双语发布 34 个学习单元：O01-O08、F01-F08、M01-M14，以及 Q01/Q03-Q05。下面是当前完整发布的严格先修关系：

R1 仍是最近一次已完成的聚合发布复核。[Issue #19](https://github.com/xiangzhang-coding/cuda-learning-site/issues/19) 范围内的 M09-M14、EX07-EX09、VIS08 及其配套学习材料已进入完整的当前增量发布范围；这不表示全部 R2 已完成，R2 聚合发布复核仍是后续发布门禁。
...
完成这些基础后，M07/M08 汇入 M09、M11 和 M14；M01/M02 汇入 M10；M05/M06 汇入 M12；M03/M05/M08 汇入 M13。EX07 从 M07/M08/M09 取得显式 copy pipeline 合同，EX08 从 M10 取得 managed-memory 合同，EX09 从 M14 取得 graph lifecycle 合同。正确性路线从 F04/O04 汇入 Q01，随后分别进入 F05/Q01 汇成的 Q03、M05/M06/Q03 汇成的 Q04，以及 M08/Q01 汇成的 Q05；Q05 再为 LAB04/LAB05 提供测量合同，Q03/Q04 则通向 EX16/LAB07。这些学习建议不增加上面没有列出的先修边。

O02-O08、F01-F08、M01-M14 与 Q01/Q03-Q05 都有直接练习和独立参考解答。新增内存路线可按顺序进入 [M09 练习](/memory/pinned-memory-transfer-overlap/exercises/)与[解答](/memory/pinned-memory-transfer-overlap/solutions/)、[M10 练习](/memory/unified-memory-page-migration/exercises/)与[解答](/memory/unified-memory-page-migration/solutions/)、[M11 练习](/memory/stream-ordered-allocation-memory-pools/exercises/)与[解答](/memory/stream-ordered-allocation-memory-pools/solutions/)、[M12 练习](/memory/cooperative-groups/exercises/)与[解答](/memory/cooperative-groups/solutions/)、[M13 练习](/memory/asynchronous-copy-pipelines/exercises/)与[解答](/memory/asynchronous-copy-pipelines/solutions/)、[M14 练习](/memory/cuda-graphs/exercises/)与[解答](/memory/cuda-graphs/solutions/)。正确性路线可直接进入 [Q01 练习](/correctness/cpu-references-tolerances-invariants/exercises/)与[解答](/correctness/cpu-references-tolerances-invariants/solutions/)、[Q03 练习](/correctness/memcheck-invalid-memory-access/exercises/)与[解答](/correctness/memcheck-invalid-memory-access/solutions/)、[Q04 练习](/correctness/racecheck-initcheck-synccheck/exercises/)与[解答](/correctness/racecheck-initcheck-synccheck/solutions/)、[Q05 练习](/correctness/timing-asynchronous-gpu-work/exercises/)与[解答](/correctness/timing-asynchronous-gpu-work/solutions/)；既有单元仍可通过上方严格先修图直接访问。

[实验索引](/labs/)按当前顺序直接列出 [LAB01](/labs/record-cuda-environment/)、[LAB02](/labs/vector-addition/)、[LAB03](/labs/break-and-repair-indexing/)、[LAB04](/labs/observe-coalescing/)、[LAB05](/labs/remove-shared-memory-bank-conflicts/)和 [LAB07](/labs/diagnose-four-sanitizer-failures/)；这些是六个完整公开实验。LAB06 不在导航中，因为所需的 timeline-profiler 先修单元尚未公开。[可视化讲解索引](/visuals/)列出十二项正式讲解：八个独立页面 [VIS01](/visuals/kernel-journey/)、[VIS02](/visuals/indexing/)、[VIS03](/visuals/warp-divergence/)、[VIS04](/visuals/memory-transactions/)、[VIS05](/visuals/shared-memory-banks/)、[VIS06](/visuals/memory-hierarchy-lifetime/)、[VIS07](/visuals/stream-event-dependencies/)与 [VIS08](/visuals/page-migration/)，以及内嵌 VIS19-VIS22。VIS08 同时依赖 M01、M02 和 M10。十二项讲解都是确定性的 browser-only model，提供静态或文字回退，不执行 CUDA，也没有 CUDA Evidence Status。当前 catalog 是 6 个 Lab、35 个[练习题库](/practice/)条目、12 项 Visual Explainer、114 项[术语表](/glossary/)和 45 项[来源与版本记录](/sources-and-versions/)，共 212 条记录；公开内容形成 131 个双语发布对（Publication Pair）和 262 条 source route。你还可以直接查阅[关于本站](/about/)。

M09-M14 与 Q01/Q03-Q05 的 compilation/runtime axes 均为空；静态学习材料、练习和 Context7 cross-check 不授予 CUDA Evidence Status。EX01 保留现有状态：没有 Compile-Checked 声明，runtime 为 Pending Hardware Verification。EX03-EX09 各自在 11.8.0、12.9.2 与 13.3.1 三条工具包通道（Toolkit Lane）使用一份原创 C++17 实现；EX16 是一个跨相同 Lanes 构建八个隔离 binary 的原创 Apache-2.0 C++17 project。EX05-EX09、EX16、LAB04、LAB05 与 LAB07 的 compilation evidence 为空，runtime 为 Pending Hardware Verification，recorded observations 为空，只提供 expected observations。Host-only checks 不编译或运行 CUDA，也不能建立 GPU correctness、overlap、migration、graph replay 或 sanitizer behavior；本站没有运行 EX03-EX09/EX16 CUDA binary、Compute Sanitizer 或 profiler，也没有记录实际 output、sanitizer report、timing、speedup 或其他 performance number。
