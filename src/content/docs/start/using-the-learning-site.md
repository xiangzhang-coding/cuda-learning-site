...
截至 **2026-08-30**，稳定课程已完整双语发布 44 个学习单元：O01-O08、F01-F08、M01-M19、A01-A04，以及 Q01-Q05。下面是当前完整发布的严格先修关系：
...
12. 并行算法路线包含 [A01：逐元素映射与一元素一所有者](/algorithms/elementwise-map/)，它同时依赖 F03、F04 和 M02；[A02：多阶段归约、屏障与运算顺序](/algorithms/multi-stage-reduction/)同时依赖 M03、M05 和 M06；[A03：包含式与排除式扫描](/algorithms/inclusive-exclusive-scan/)同时依赖 A02 和 M05；[A04：私有化直方图](/algorithms/privatized-histogram/)同时依赖 M03 和 M05。
13. 正确性与质量路线包含 [Q01：CPU 参考实现、容差与不变量](/correctness/cpu-references-tolerances-invariants/)，它同时依赖 F04 和 O04；[Q02：浮点顺序、确定性与逐位可复现性](/correctness/floating-point-order-reproducibility/)同时依赖 Q01 和 A02；[Q03：用 memcheck 定位非法内存访问](/correctness/memcheck-invalid-memory-access/)同时依赖 F05 和 Q01；[Q04：用 racecheck、initcheck 与 synccheck 定位缺陷](/correctness/racecheck-initcheck-synccheck/)同时依赖 M05、M06 和 Q03；[Q05：诚实计时异步 GPU 工作](/correctness/timing-asynchronous-gpu-work/)同时依赖 M08 和 Q01。
14. [EX03 多维索引可运行示例](/examples/multidimensional-indexing/)依赖 F03，并为 F03/F04 提供 canonical source；[EX04 错误处理生命周期可运行示例](/examples/error-handling-lifecycle/)仅依赖 F05；[EX05 合并与跨步访问可运行示例](/examples/coalesced-strided-access/)仅依赖 M02；[EX06 共享内存 tile bank padding 可运行示例](/examples/shared-memory-tile-bank-padding/)同时依赖 M03 和 M04；[EX07 Streams、Events 与 Overlap 可运行示例](/examples/streams-events-overlap/)同时依赖 M07、M08 和 M09；[EX08 统一内存迁移可运行示例](/examples/unified-memory-migration/)仅依赖 M10；[EX09 CUDA Graph Capture 可运行示例](/examples/graph-capture/)仅依赖 M14；[EX10 PTX 与 Fatbinary 检查可运行示例](/examples/ptx-fatbinary-inspection/)同时依赖 M15 和 M16；[EX11 多阶段归约可运行示例](/examples/multi-stage-reduction/)同时依赖 A02 和 Q02；[EX12 Inclusive 与 Exclusive Scan 可运行示例](/examples/inclusive-exclusive-scan/)仅依赖 A03；[EX13 私有化 Histogram 可运行示例](/examples/privatized-histogram/)仅依赖 A04；[EX16 Compute Sanitizer 缺陷套件](/examples/sanitizer-defect-suite/)同时依赖 Q03 和 Q04。十四个 Runnable Example 严格是 EX01-EX13 与 EX16；EX14 和 EX15 不存在公开目标。[LAB02](/labs/vector-addition/)同时依赖 O03 和 F01，并使用 canonical [EX02](/examples/vector-addition/)；[LAB03：破坏并修复索引](/labs/break-and-repair-indexing/)同时依赖 F03 和 F05，并使用 EX04；[LAB04：观察合并访问](/labs/observe-coalescing/)同时依赖 M02 和 Q05，并使用 EX05；[LAB05：消除共享内存存储体冲突](/labs/remove-shared-memory-bank-conflicts/)同时依赖 M04 和 Q05，并使用 EX06；[LAB07：诊断四类 Sanitizer 故障](/labs/diagnose-four-sanitizer-failures/)同时依赖 Q03 和 Q04，并使用 EX16。六个非连续 Lab 严格是 LAB01-LAB05 与 LAB07；LAB06 继续缺席，因为所需的 timeline-profiler 先修单元尚未公开。F08 与 LAB03 相关，但不是 LAB03 的先修条件。
...
完成这些基础后，M07/M08 汇入 M09、M11 和 M14；M01/M02 汇入 M10；M05/M06 汇入 M12；M03/M05/M08 汇入 M13。EX07 从 M07/M08/M09 取得显式 copy pipeline 合同，EX08 从 M10 取得 managed-memory 合同，EX09 从 M14 取得 graph lifecycle 合同。工具链路线从 F04/O04 汇入 M15，再由 M15/F06 汇入 M16、M16/F06 汇入 M17、M15/M16 汇入 M18、O04/M15 汇入 M19；EX10 严格依赖 M15/M16，VIS09 严格依赖 M15/M16/M17。算法路线从 F03/F04/M02 汇入 A01，从 M03/M05/M06 汇入 A02，再由 A02/M05 汇入 A03、M03/M05 汇入 A04；VIS10 严格依赖 A02。正确性路线从 F04/O04 汇入 Q01，并由 Q01/A02 汇入 Q02；EX11 严格依赖 A02/Q02，EX12 依赖 A03，EX13 依赖 A04。其余正确性分支分别进入 F05/Q01 汇成的 Q03、M05/M06/Q03 汇成的 Q04，以及 M08/Q01 汇成的 Q05；Q05 再为 LAB04/LAB05 提供测量合同，Q03/Q04 则通向 EX16/LAB07。这些学习建议不增加上面没有列出的先修边。

O02-O08、F01-F08、M01-M19 与 Q01/Q03-Q05 都有直接练习和独立参考解答。内存路线可按顺序进入 [M09 练习](/memory/pinned-memory-transfer-overlap/exercises/)与[解答](/memory/pinned-memory-transfer-overlap/solutions/)、[M10 练习](/memory/unified-memory-page-migration/exercises/)与[解答](/memory/unified-memory-page-migration/solutions/)、[M11 练习](/memory/stream-ordered-allocation-memory-pools/exercises/)与[解答](/memory/stream-ordered-allocation-memory-pools/solutions/)、[M12 练习](/memory/cooperative-groups/exercises/)与[解答](/memory/cooperative-groups/solutions/)、[M13 练习](/memory/asynchronous-copy-pipelines/exercises/)与[解答](/memory/asynchronous-copy-pipelines/solutions/)、[M14 练习](/memory/cuda-graphs/exercises/)与[解答](/memory/cuda-graphs/solutions/)。工具链路线可进入 [M15 练习](/toolchain/nvcc-compilation-flow/exercises/)与[解答](/toolchain/nvcc-compilation-flow/solutions/)、[M16 练习](/toolchain/ptx-cubin-fatbinary/exercises/)与[解答](/toolchain/ptx-cubin-fatbinary/solutions/)、[M17 练习](/toolchain/compiler-architecture-targets/exercises/)与[解答](/toolchain/compiler-architecture-targets/solutions/)、[M18 练习](/toolchain/separate-compilation-device-linking/exercises/)与[解答](/toolchain/separate-compilation-device-linking/solutions/)、[M19 练习](/toolchain/cpp-dialect-boundaries/exercises/)与[解答](/toolchain/cpp-dialect-boundaries/solutions/)。正确性路线可直接进入 [Q01 练习](/correctness/cpu-references-tolerances-invariants/exercises/)与[解答](/correctness/cpu-references-tolerances-invariants/solutions/)、[Q03 练习](/correctness/memcheck-invalid-memory-access/exercises/)与[解答](/correctness/memcheck-invalid-memory-access/solutions/)、[Q04 练习](/correctness/racecheck-initcheck-synccheck/exercises/)与[解答](/correctness/racecheck-initcheck-synccheck/solutions/)、[Q05 练习](/correctness/timing-asynchronous-gpu-work/exercises/)与[解答](/correctness/timing-asynchronous-gpu-work/solutions/)；既有单元仍可通过上方严格先修图直接访问。

算法路线可进入 [A01 练习](/algorithms/elementwise-map/exercises/)与[解答](/algorithms/elementwise-map/solutions/)、[A02 练习](/algorithms/multi-stage-reduction/exercises/)与[解答](/algorithms/multi-stage-reduction/solutions/)、[A03 练习](/algorithms/inclusive-exclusive-scan/exercises/)与[解答](/algorithms/inclusive-exclusive-scan/solutions/)、[A04 练习](/algorithms/privatized-histogram/exercises/)与[解答](/algorithms/privatized-histogram/solutions/)；Q02 也提供[练习](/correctness/floating-point-order-reproducibility/exercises/)与独立[解答](/correctness/floating-point-order-reproducibility/solutions/)。

[实验索引](/labs/)按当前顺序直接列出 [LAB01](/labs/record-cuda-environment/)、[LAB02](/labs/vector-addition/)、[LAB03](/labs/break-and-repair-indexing/)、[LAB04](/labs/observe-coalescing/)、[LAB05](/labs/remove-shared-memory-bank-conflicts/)和 [LAB07](/labs/diagnose-four-sanitizer-failures/)；这些是六个完整公开实验。LAB06 不在导航中，因为所需的 timeline-profiler 先修单元尚未公开。[可视化讲解索引](/visuals/)列出十四项正式讲解：十个独立页面 [VIS01](/visuals/kernel-journey/)、[VIS02](/visuals/indexing/)、[VIS03](/visuals/warp-divergence/)、[VIS04](/visuals/memory-transactions/)、[VIS05](/visuals/shared-memory-banks/)、[VIS06](/visuals/memory-hierarchy-lifetime/)、[VIS07](/visuals/stream-event-dependencies/)、[VIS08](/visuals/page-migration/)、[VIS09](/visuals/artifact-pipeline/)与 [VIS10](/visuals/reduction-stages/)，以及内嵌 VIS19-VIS22。VIS08 同时依赖 M01、M02 和 M10；VIS09 同时依赖 M15、M16 和 M17；VIS10 仅依赖 A02。十四项讲解都是确定性的 browser-only model，提供静态或文字回退，不执行 CUDA，也没有 CUDA Evidence Status；VIS10 的静态 stage diagram 回退同样保持证据中立。当前 catalog 是 6 个 Lab、45 个[练习题库](/practice/)条目、14 项 Visual Explainer、139 项[术语表](/glossary/)和 56 项[来源与版本记录](/sources-and-versions/)，共 260 条记录；公开内容形成 167 个双语发布对（Publication Pair）和 334 条 source route。你还可以直接查阅[关于本站](/about/)。

M09-M19、A01-A04 与 Q01-Q05 的 compilation/runtime axes 均为空；静态学习材料、练习和 Context7 cross-check 不授予 CUDA Evidence Status。EX01 保留现有状态：没有 Compile-Checked 声明，runtime 为 Pending Hardware Verification。EX03-EX09 与 EX11-EX13 各自在 11.8.0、12.9.2 与 13.3.1 三条工具包通道（Toolkit Lane）使用一份原创 C++17 实现；EX16 是一个跨相同 Lanes 构建八个隔离 binary 的原创 Apache-2.0 C++17 project。EX05-EX09、EX11-EX13、EX16、LAB04、LAB05 与 LAB07 的 compilation evidence 为空，runtime 为 Pending Hardware Verification，recorded observations 为空，只提供 expected observations。EX10 在 run 33275734951 中保留五项 ordinary Compile-Checked records；它只生成和检查构建产物，不执行最终 host artifact 或 GPU executable，因此 runtime 为 Runtime-Not-Applicable。独立 CUDA 13.3.1/NVCC 13.3.73/GCC 14.2.0 C++23 probe 是 narrow pass，不声明 ordinary C++23 support、其他 compiler/platform、runtime 或 performance。Host-only checks 不编译或运行 CUDA，也不能建立 GPU correctness、overlap、migration、graph replay、scan、histogram、reduction runtime behavior 或 performance；本站没有运行 EX03-EX13/EX16 CUDA binary、Compute Sanitizer 或 profiler，也没有记录实际 output、sanitizer report、timing、speedup 或其他 performance number。
...
**事实核查日期：2026-08-30。** 本页不依赖特定 CUDA Toolkit 版本，也不会授予 CUDA 证据状态。术语定义见[术语表](/glossary/)，发布接口与 CUDA 来源依据见[来源与版本记录](/sources-and-versions/)。
