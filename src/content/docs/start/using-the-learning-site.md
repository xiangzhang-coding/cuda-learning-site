...
截至 **2026-08-29**，稳定课程已完整双语发布 39 个学习单元：O01-O08、F01-F08、M01-M19，以及 Q01/Q03-Q05。下面是当前完整发布的严格先修关系：
...
完成这些基础后，M07/M08 汇入 M09、M11 和 M14；M01/M02 汇入 M10；M05/M06 汇入 M12；M03/M05/M08 汇入 M13。EX07 从 M07/M08/M09 取得显式 copy pipeline 合同，EX08 从 M10 取得 managed-memory 合同，EX09 从 M14 取得 graph lifecycle 合同。工具链路线从 F04/O04 汇入 M15，再由 M15/F06 汇入 M16、M16/F06 汇入 M17、M15/M16 汇入 M18、O04/M15 汇入 M19；EX10 严格依赖 M15/M16，VIS09 严格依赖 M15/M16/M17。正确性路线从 F04/O04 汇入 Q01，随后分别进入 F05/Q01 汇成的 Q03、M05/M06/Q03 汇成的 Q04，以及 M08/Q01 汇成的 Q05；Q05 再为 LAB04/LAB05 提供测量合同，Q03/Q04 则通向 EX16/LAB07。这些学习建议不增加上面没有列出的先修边。
...
[实验索引](/labs/)按当前顺序直接列出 [LAB01](/labs/record-cuda-environment/)、[LAB02](/labs/vector-addition/)、[LAB03](/labs/break-and-repair-indexing/)、[LAB04](/labs/observe-coalescing/)、[LAB05](/labs/remove-shared-memory-bank-conflicts/)和 [LAB07](/labs/diagnose-four-sanitizer-failures/)；这些是六个完整公开实验。LAB06 不在导航中，因为所需的 timeline-profiler 先修单元尚未公开。[可视化讲解索引](/visuals/)列出十三项正式讲解：九个独立页面 [VIS01](/visuals/kernel-journey/)、[VIS02](/visuals/indexing/)、[VIS03](/visuals/warp-divergence/)、[VIS04](/visuals/memory-transactions/)、[VIS05](/visuals/shared-memory-banks/)、[VIS06](/visuals/memory-hierarchy-lifetime/)、[VIS07](/visuals/stream-event-dependencies/)、[VIS08](/visuals/page-migration/)与 [VIS09](/visuals/artifact-pipeline/)，以及内嵌 VIS19-VIS22。VIS08 同时依赖 M01、M02 和 M10；VIS09 同时依赖 M15、M16 和 M17。十三项讲解都是确定性的 browser-only model，提供静态或文字回退，不执行 CUDA，也没有 CUDA Evidence Status。当前 catalog 是 6 个 Lab、40 个[练习题库](/practice/)条目、13 项 Visual Explainer、125 项[术语表](/glossary/)和 50 项[来源与版本记录](/sources-and-versions/)，共 234 条记录；公开内容形成 148 个双语发布对（Publication Pair）和 296 条 source route。你还可以直接查阅[关于本站](/about/)。

M09-M19 与 Q01/Q03-Q05 的 compilation/runtime axes 均为空；静态学习材料、练习和 Context7 cross-check 不授予 CUDA Evidence Status。EX01 保留现有状态：没有 Compile-Checked 声明，runtime 为 Pending Hardware Verification。EX03-EX09 各自在 11.8.0、12.9.2 与 13.3.1 三条工具包通道（Toolkit Lane）使用一份原创 C++17 实现；EX16 是一个跨相同 Lanes 构建八个隔离 binary 的原创 Apache-2.0 C++17 project。EX05-EX09、EX16、LAB04、LAB05 与 LAB07 的 compilation evidence 为空，runtime 为 Pending Hardware Verification，recorded observations 为空，只提供 expected observations。EX10 在 run 33266515216 中保留五项 ordinary Compile-Checked records；它只生成和检查构建产物，不执行最终 host artifact 或 GPU executable，因此 runtime 为 Runtime-Not-Applicable。独立 CUDA 13.3.1/NVCC 13.3.73/GCC 14.2.0 C++23 probe 是 narrow pass，不声明 ordinary C++23 support、其他 compiler/platform、runtime 或 performance。Host-only checks 不编译或运行 CUDA，也不能建立 GPU correctness、overlap、migration、graph replay 或 sanitizer behavior；本站没有运行 EX03-EX10/EX16 CUDA binary、Compute Sanitizer 或 profiler，也没有记录实际 output、sanitizer report、timing、speedup 或其他 performance number。
...
**事实核查日期：2026-08-29。** 本页不依赖特定 CUDA Toolkit 版本，也不会授予 CUDA 证据状态。术语定义见[术语表](/glossary/)，发布接口与 CUDA 来源依据见[来源与版本记录](/sources-and-versions/)。
