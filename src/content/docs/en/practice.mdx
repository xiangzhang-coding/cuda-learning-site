...
A planning ID is a stable label for cross-curriculum discussion, not a URL-stability promise. Filtering changes only the cards visible in the current browser. The five complete prompts, prerequisite links, and solutions below remain in the static page.
...
All five entries and their Chinese counterparts were reviewed together on **2026-08-25**. Every scenario, prompt, and solution is original project work. No external question, output, log, or performance figure was copied, and no CUDA execution occurred.
