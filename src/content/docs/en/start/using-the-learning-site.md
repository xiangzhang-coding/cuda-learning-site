...
As of **2026-08-28**, the completely published strict prerequisite graph is:
...
10. The [EX03 Multidimensional Indexing Runnable Example](/en/examples/multidimensional-indexing/) depends on F03 and supplies canonical source to F03/F04; the [EX04 Error Handling Lifecycle Runnable Example](/en/examples/error-handling-lifecycle/) depends only on F05; the [EX05 Coalesced and Strided Access Runnable Example](/en/examples/coalesced-strided-access/) depends only on M02; and the [EX06 Shared-Memory Tile Bank Padding Runnable Example](/en/examples/shared-memory-tile-bank-padding/) requires M03 and M04. Runnable Examples remain exactly EX01-EX06, with no EX07. [LAB02](/en/labs/vector-addition/) requires O03 and F01 and uses canonical [EX02](/en/examples/vector-addition/); [LAB03: Break and Repair Indexing](/en/labs/break-and-repair-indexing/) requires F03 and F05 and uses EX04. F08 is related to LAB03 but is not a LAB03 prerequisite; Labs remain exactly LAB01-LAB03.

Choose the next step by the gap you need to close. Apply the O04 C++17 refresher while reading F01-F08 and EX02-EX06, but do not add it as a prerequisite. The Linux-record route must combine O02, O03, and O05 before O08. EX01 remains directly available, while LAB01 requires both O03 and O08. The architecture route follows O06 with O07 before carrying those models into Foundations. F05 continues the error lifecycle from F04; F06 joins F02/O03; F07 joins F04/F05; and F08 joins F02/F03/F06. The memory route joins F04/F06 at M01, uses M02-M04 for memory access and layout, then enters M05/M06 for synchronization and warp reasoning, while F05/M01 join at M07 before M08 adds event dependencies and timing. These recommendations add no prerequisite edge beyond the graph above.

O02-O08, F01-F08, and M01-M08 all have direct Exercises and separate reviewed solutions. The new memory route links directly to [M05 Exercises](/en/memory/synchronization-scopes/exercises/) and [solutions](/en/memory/synchronization-scopes/solutions/), [M06 Exercises](/en/memory/warp-divergence-reconvergence/exercises/) and [solutions](/en/memory/warp-divergence-reconvergence/solutions/), [M07 Exercises](/en/memory/stream-ordering/exercises/) and [solutions](/en/memory/stream-ordering/solutions/), and [M08 Exercises](/en/memory/event-dependencies-timing/exercises/) and [solutions](/en/memory/event-dependencies-timing/solutions/). Earlier units remain directly reachable through the strict graph above.

The [Lab Index](/en/labs/) directly lists [LAB01](/en/labs/record-cuda-environment/), [LAB02](/en/labs/vector-addition/), and [LAB03](/en/labs/break-and-repair-indexing/) in the current order. These are exactly the three public Labs, and navigation exposes no unfinished Learning Unit. The [Visual Explainer Index](/en/visuals/) lists eleven formal explainers: seven standalone pages, [VIS01](/en/visuals/kernel-journey/), [VIS02](/en/visuals/indexing/), [VIS03](/en/visuals/warp-divergence/), [VIS04](/en/visuals/memory-transactions/), [VIS05](/en/visuals/shared-memory-banks/), [VIS06](/en/visuals/memory-hierarchy-lifetime/), and [VIS07](/en/visuals/stream-event-dependencies/), plus embedded VIS19-VIS22. All eleven are deterministic browser-only models with static or textual fallbacks; they execute no CUDA and have no CUDA Evidence Status. The final catalog has 3 Labs, 25 [Practice Bank](/en/practice/) entries, 11 Visual Explainers, 86 [Glossary](/en/glossary/) terms, and 36 [Sources and Version Record](/en/sources-and-versions/) entries, for 161 total. Public content forms 93 Publication Pairs and 186 source routes. You can also open [About](/en/about/) directly.

EX01 retains its existing status: no Compile-Checked claim and Pending Hardware Verification runtime. EX03-EX06 each use one original C++17 implementation across Toolkit Lanes 11.8.0, 12.9.2, and 13.3.1. EX05 and EX06 have no qualifying retained compile records, so compilation evidence is empty; runtime is Pending Hardware Verification, recorded observations are empty, and only expected observations are published. Host-only checks compile or execute no CUDA. The site ran no EX03-EX06 CUDA binary and records no actual output, timing, speedup, or other performance number for them.
...
**Fact-check date: 2026-08-28.** This page is not tied to a CUDA Toolkit version and does not grant CUDA evidence. See the [Glossary](/en/glossary/) for vocabulary and [Sources and Version Record](/en/sources-and-versions/) for publishing and CUDA sources reviewed for this release.
