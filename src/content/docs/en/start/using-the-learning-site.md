...
**Current architecture increment, 2026-09-22 (#59):** [H01](/en/architecture/turing-warp-safety/) requires `[F06,M06]`, and [H02](/en/architecture/ampere-pipelines-tensor-cores/) requires `[H01,M13,L08]`. Both have separate Exercises/solutions; [VIS15](/en/visuals/architecture-evolution/) compares Turing and Ampere. Current scope: 106 Learning Units, 24 Runnable Examples, 18 Labs, 22 Visual Explainers, 105 Exercise sets, 105 separate solution sets, 383 Publication Pairs, 766 source routes and 483 catalog records (117 Practice Bank entries, 207 Glossary terms, 119 source records, plus Labs/visuals). Existing 41 subjects remain Pending Hardware Verification. Architecture execution/performance is unobserved. R6 inventories below are frozen historical scope; R7 remains pending.
...
The latest completed aggregate static review is **R6, dated 2026-09-22**. Frozen `/release.json` includes G01-G09 and all prior paths: 104 units, 376 Publication Pairs, 752 routes, 103 Exercise sets, 103 separate solution sets and 478 catalog records. Its 115 original questions include 13 multi-GPU and NCCL entries. Current `/publication.json` additionally tracks H01/H02/VIS15. R1-R5 remain historical snapshots and R7 is next/pending; publication does not upgrade runtime evidence.
...
16. The 23 Runnable Examples are EX01-EX23; [EX21](/en/examples/cuda-python-launch/) retains `[P02]`, [EX20](/en/examples/cusparse-spmv/) retains `[L13]`, [EX19](/en/examples/cufft-batched-transform/) retains `[L12]`, and [EX18](/en/examples/cublas-gemm/) retains `[L06]`. The 16 Labs are LAB01-LAB16; [LAB12](/en/labs/compare-gemm-with-cublas/) retains `[Q13,L06]`. [EX22](/en/examples/adjacent-energy/) and [LAB13](/en/labs/build-custom-operator/) both require `[P08,P09]`. [LAB14](/en/labs/profile-custom-operator/) requires `[P11]`; [LAB15](/en/labs/verify-fused-softmax/) requires `[T02,A10,Q05]`; [LAB16](/en/labs/autotune-triton-gemm/) requires `[T04,T05]`. [VIS18](/en/visuals/attention-memory-traffic/) still requires `[A11]`. [EX23](/en/examples/triton-vector-add/) requires `[T02]`, and [VIS17](/en/visuals/simt-triton-mapping/) requires `[T01]`.
...
The current 104 Learning Units include [G07](/en/multi-gpu/pytorch-ddp-nccl/) with prerequisites `[P04,P05,G04]`, [G08](/en/multi-gpu/nccl-graph-capture/) with `[G05,M14]` and [G09](/en/multi-gpu/multi-node-transport-failures/) with `[G03,G04,Q07]`. There are 24 Runnable Examples. The [Lab Index](/en/labs/) lists 18 Labs and the [Visual Explainer Index](/en/visuals/) lists 21 explainers. There are 115 [Practice Bank](/en/practice/) entries, 207 [Glossary](/en/glossary/) terms and 117 [source records](/en/sources-and-versions/): 478 catalog records. SRC-CUDA-102 records exact multi-node transport/failure contracts; the ten R3 Nsight report-analysis and eight R4 library-and-algorithm-choice entries remain unchanged. There are 41 subjects Pending Hardware Verification. G09's four evidence arrays are empty and multi-node execution is unobserved.
...
Only EX02, EX10 and LAB02 are Compile-Checked. EX10 is Runtime-Not-Applicable; 23 other examples and 18 Labs remain Pending Hardware Verification, 41 subjects total. P01-P12, T01-T08 and G01-G06 keep empty evidence arrays. EX21-EX24 and LAB13-LAB18 remain Pending Hardware Verification with empty compilation/recorded observations. Runtime-Verified, Community-Observed, Reference Environment and performance-observation counts stay zero. Source review and host checks are not GPU evidence.
