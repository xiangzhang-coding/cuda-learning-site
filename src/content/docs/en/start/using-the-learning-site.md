...
The R3 Stable Curriculum reviewed in [issue #32](https://github.com/xiangzhang-coding/cuda-learning-site/issues/32) completely publishes 62 Learning Units in both languages: O01-O08, F01-F08, M01-M19, A01-A14, and Q01-Q13. Rolling R4 extends through L01-L12, EX17/EX18/EX19, and LAB11/LAB12; [issue #39](https://github.com/xiangzhang-coding/cuda-learning-site/issues/39) adds [L12: cuFFT Plans, Layouts, and Startup](/en/libraries/cufft-plans-layouts-startup/) and [EX19: cuFFT Batched Transform](/en/examples/cufft-batched-transform/), bringing the current total to 74 Learning Units. The complete strict prerequisite graph currently published follows.

The completed R3 aggregate release review is an immutable snapshot fixed at 232 Publication Pairs, 464 source routes, and 347 catalog records. It also inventories 61 Exercise sets, 61 separate reviewed-solution sets, and 10 Nsight report-analysis Practice Bank entries. The rolling current publication has 273 Publication Pairs, 546 source routes, 73 Exercise sets, 73 separate reviewed-solution sets, and 395 catalog records. R1 and R2 remain historical coordinates, and the R4 aggregate review remains pending.

**Publication inventory and navigation updated: 2026-09-08.** The page date covers only this inventory update, not a new review of existing CUDA API facts; technical sources retain their access and review dates.
...
16. The Runnable Example set is EX01-EX19, ending with [EX19: cuFFT Batched Transform](/en/examples/cufft-batched-transform/)`<-[L12]`. [EX18: cuBLAS GEMM](/en/examples/cublas-gemm/) still requires `[L06]`. The Lab set is LAB01-LAB12, ending with [LAB12: Compare an Educational GEMM with cuBLAS](/en/labs/compare-gemm-with-cublas/)`<-[Q13,L06]`. The visual graph retains [VIS18: Attention Memory Traffic](/en/visuals/attention-memory-traffic/)`<-[A11]`. L13 and EX20 remain pending.
...
**Publication inventory checked: 2026-09-08.** This page is not tied to a CUDA Toolkit version and does not grant CUDA evidence. See the [Glossary](/en/glossary/) for vocabulary and [Sources and Version Record](/en/sources-and-versions/) for publishing and CUDA sources reviewed for this release.
