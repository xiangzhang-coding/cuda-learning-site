...
16. The 20 Runnable Examples are EX01-EX20, ending with [EX20: cuSPARSE SpMV](/en/examples/cusparse-spmv/)`<-[L13]`. [EX19: cuFFT Batched Transform](/en/examples/cufft-batched-transform/) still requires `[L12]`, and [EX18: cuBLAS GEMM](/en/examples/cublas-gemm/) still requires `[L06]`. The Lab set is LAB01-LAB12, ending with [LAB12: Compare an Educational GEMM with cuBLAS](/en/labs/compare-gemm-with-cublas/)`<-[Q13,L06]`. The visual graph retains [VIS18: Attention Memory Traffic](/en/visuals/attention-memory-traffic/)`<-[A11]`. L13/EX20 add no Lab.
