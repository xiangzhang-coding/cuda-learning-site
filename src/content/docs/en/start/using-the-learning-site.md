...
8. The [EX03 Multidimensional Indexing Runnable Example](/en/examples/multidimensional-indexing/) depends on F03 and supplies canonical source to F03/F04. [LAB02](/en/labs/vector-addition/) still requires both O03 and F01 and uses canonical [EX02](/en/examples/vector-addition/).

Choose the next step by the gap you need to close. Apply the O04 C++17 refresher while reading F01-F04 and EX02/EX03, but do not add it as a prerequisite. The Linux-record route must combine O02, O03, and O05 before O08. EX01 remains directly available, while LAB01 requires both O03 and O08. The architecture route follows O06 with O07 before carrying those models into F01-F04. F02-F03 form a continuous indexing route, while F04 branches from F01. These recommendations add no prerequisite edge beyond the graph above.

O02-O08 and F01-F04 all have direct Exercises and separate reviewed solutions. The foundation route links directly to [F01 Exercises](/en/foundations/first-cuda-kernel/exercises/) and [solutions](/en/foundations/first-cuda-kernel/solutions/), [F02 Exercises](/en/foundations/execution-hierarchy/exercises/) and [solutions](/en/foundations/execution-hierarchy/solutions/), [F03 Exercises](/en/foundations/multidimensional-indexing/exercises/) and [solutions](/en/foundations/multidimensional-indexing/solutions/), and [F04 Exercises](/en/foundations/host-device-lifecycle/exercises/) and [solutions](/en/foundations/host-device-lifecycle/solutions/).

The [Lab Index](/en/labs/) still lists only LAB01 and LAB02 in prerequisite order; navigation exposes no LAB03 or unfinished Learning Unit. The [Visual Explainer Index](/en/visuals/) continues to reuse the deterministic browser models, text, and static fallbacks for VIS01 Kernel Journey and VIS02 Indexing. F04's original static lifecycle table is not a new Visual Explainer and creates no evidence. The [Practice Bank](/en/practice/) now contains thirteen complete entries linked back to O02-O08 and F01-F04. You can also open the expanded [Glossary](/en/glossary/), [Sources and Version Record](/en/sources-and-versions/), and [About](/en/about/) directly. If learning material is absent from navigation, it is not public; an identifier or mention does not imply that a page exists.

EX03 uses the same original C++17 source across Toolkit Lanes 11.8.0, 12.9.2, and 13.3.1. Host-only checks compile or execute no CUDA. EX03's compilation evidence is empty, so it is not Compile-Checked, and runtime is Pending Hardware Verification. The site ran no EX03 CUDA binary and records no output, timing, or performance number.
