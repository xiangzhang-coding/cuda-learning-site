...
As of **2026-09-05**, the R3 Stable Curriculum reviewed in [issue #32](https://github.com/xiangzhang-coding/cuda-learning-site/issues/32) completely publishes 62 Learning Units in both languages: O01-O08, F01-F08, M01-M19, A01-A14, and Q01-Q13. The rolling current publication adds L01-L02 through issue #33, [L03](/en/libraries/cub-device-primitives/), [L04](/en/libraries/cub-warp-block-primitives/), [EX17](/en/examples/cub-device-reduction-scan/), and [LAB11](/en/labs/compare-custom-reduction-with-cub/) through [issue #34](https://github.com/xiangzhang-coding/cuda-learning-site/issues/34), then [L05: libcu++ Synchronization Abstractions](/en/libraries/libcu-plus-plus-synchronization/). [Issue #36](https://github.com/xiangzhang-coding/cuda-learning-site/issues/36) adds [L06: cuBLAS GEMM](/en/libraries/cublas-gemm/), [L07: cuBLASLt Matmul](/en/libraries/cublaslt-matmul/), [EX18](/en/examples/cublas-gemm/), and [LAB12](/en/labs/compare-gemm-with-cublas/); [issue #37](https://github.com/xiangzhang-coding/cuda-learning-site/issues/37) adds [L08: Tensor Core Precision and Architecture Contracts](/en/libraries/tensor-core-precision-contracts/) and [L09: CUTLASS C++ GEMM Structure](/en/libraries/cutlass-cpp-gemm-structure/), bringing the current total to 71 Learning Units. The complete strict prerequisite graph currently published follows.

The completed R3 aggregate release review is an immutable snapshot fixed at 232 Publication Pairs, 464 source routes, and 347 catalog records. It also inventories 61 Exercise sets, 61 separate reviewed-solution sets, and 10 Nsight report-analysis Practice Bank entries. The rolling current publication has 263 Publication Pairs, 526 source routes, 70 Exercise sets, 70 separate reviewed-solution sets, and 383 catalog records. R1 and R2 remain historical coordinates, and the R4 aggregate review remains pending.

**Publication inventory and navigation updated: 2026-09-07.** The page date covers only this inventory update, not a new review of existing CUDA API facts; technical sources retain their access and review dates.
...
16. The Runnable Example set is EX01-EX18, ending with [EX18: cuBLAS GEMM](/en/examples/cublas-gemm/)`<-[L06]`. The Lab set is LAB01-LAB12, ending with [LAB12: Compare an Educational GEMM with cuBLAS](/en/labs/compare-gemm-with-cublas/)`<-[Q13,L06]`. The visual graph retains [VIS18: Attention Memory Traffic](/en/visuals/attention-memory-traffic/)`<-[A11]`. L10-L13 and EX20 remain pending.
...
**Publication inventory checked: 2026-09-07.** This page is not tied to a CUDA Toolkit version and does not grant CUDA evidence. See the [Glossary](/en/glossary/) for vocabulary and [Sources and Version Record](/en/sources-and-versions/) for publishing and CUDA sources reviewed for this release.
