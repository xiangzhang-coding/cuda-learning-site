...
**Constraints:** One designated initializer sets the barrier's expected count to four, followed by an independent all-block initialization-visibility boundary. A, B, and C remain participants. D uses `arrive_and_drop` in the first ready phase and neither reads the tile nor rejoins later. Use the default completion behavior and ordinary waits; do not introduce custom completion or asynchronous copies. Distinguish a ready phase from a consumed phase, and place every B/C consumed-phase arrival after its final tile read. Do not reinitialize the barrier between tiles.
...
**Expected evidence:** An initialization/lifetime ledger; four phase rows named ready-0, consumed-0, ready-1, and consumed-1; each row's expected count, arriving participants, drop contribution, next count, permitted reads, and earliest permitted overwrite; plus a counterexample schedule in which B is slower than A. This is a possible ordering argument, not an observed trace.

**Acceptance criteria:** The first ready phase has four contributions, including D's drop, and subsequent phases expect three. Neither B nor C reads before its ready wait completes. A cannot overwrite until the consumed phase accounts for both readers' final reads. D never contributes to the three-participant phases, and the barrier and tile remain alive until their users and waits are finished. The counterexample explains why readiness alone does not release storage.
...
<details><summary>Hint 2</summary>Let A return from the ready wait while B has not yet read the tile. Which later event proves that B's last read is over, and which phase must A wait for?</details>
