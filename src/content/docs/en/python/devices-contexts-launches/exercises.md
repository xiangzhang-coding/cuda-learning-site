...
**Transfer task:** Annotate the same CUDA C++ responsibilities in a Python plan: `cudaSetDevice`, `cudaStreamCreateWithFlags`, `cudaMalloc` or `cudaMallocAsync`, `cudaMallocHost`, `cudaMemcpyAsync`, a typed C++ kernel launch, and `cudaStreamSynchronize` before CPU comparison. Use columns for the C++ API/handle, core call or object, owner/last use, and a semantic difference that prevents mechanical substitution. Also contrast Runtime-managed kernel identity with a Driver `CUmodule` / `CUfunction` and core's Kernel/library ownership. Do not implement the hypothetical kernel or change EX21.

**Expected evidence:** Repair every byte count, pointer/scalar representation, launch dimension, copy destination, and view ownership. Derive the first five outputs, useful/tail threads, and total payload by memory location. Explain the independent handling of n=0, negative n, and n greater than the uint32 range. Submit the matched CUDA C++ responsibility-to-Python table with concrete byte counts, scalar widths, and completion placement for this request, not merely a list of Python API names.

**Acceptance criteria:** All intended indices have exactly one writing thread and no tail access. Every transfer uses equally sized Buffers and explicit host destinations. A host view is only made over host-accessible storage whose owner remains alive. Raw Python scalar packing is not accepted as a signature match. Compare every finite result after `s.sync()`; do not treat five matching sample values as a full-run verdict. The transfer table must preserve C++ ownership/completion responsibilities while explicitly distinguishing primary-context selection, allocator fallback, whole-Buffer copy extents, kernel-handle ownership, and sized Python scalars from one-to-one Runtime calls.
...
**Transfer task:** A C++ owner normally pairs its allocations with `cudaFree` / `cudaFreeAsync` or `cudaFreeHost`, and its stream with `cudaStreamDestroy`; a separately owned Driver retain or module has its own release obligation. Annotate which of these responsibilities survive in the core plan and which raw-release permissions do not transfer to a borrower. Distinguish balancing an independently acquired `CUcontext` retain from resetting a shared primary context.

**Expected evidence:** Classify acquired, borrowed, and shared resources; correct the meaning of ctx; draw the normal completed path and the failed-completion path. Identify the original error and secondary diagnostics to preserve, including native stderr as well as Python exception/warning channels. State which resources have public close operations and when they can be used, why cleanup cannot establish a valid output after failed completion, and how the repaired sequence carries the C++ last-use obligations into Python.

Annotate the capture boundary as well: a diagnostic emitted during an explicit `cleanup_step` belongs to that action's verdict, whereas a destructor delayed until interpreter shutdown is outside it. State what a 16,384-byte retained payload limit does and does not bound, and why a clean capture cannot certify later GC or all eventual teardown.

**Acceptance criteria:** Do not close the stream before stream-dependent Buffer release, double-free through raw handles, unload core-owned libraries, or reset shared primary state. Use `sync()`, not `synchronize()`. Preserve failure and exit nonzero; cleanup diagnostics and failed drains must not be converted into successful verification. Do not assume `del` guarantees immediate native destruction or that every native cleanup diagnostic becomes a Python exception. Explain the difference between the C++ owner's release calls and core's owned Buffer/Stream closure, shared Kernel/ObjectCode lifetime, and primary-context retention.
