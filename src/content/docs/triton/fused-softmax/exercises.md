...
**目标：** 保持实验签名，实现每行一个程序实例（program instance）的 `normalize_rows`。看代码前解释三个相等的 −1000 分数（logits）以及 `[1000,1001,1002]` 的输出。
...
<details><summary>提示一：赋予填充值中性作用</summary>最大值必须忽略无效位置，它们的指数贡献必须为零。分数零通常不是中性值。</details>
