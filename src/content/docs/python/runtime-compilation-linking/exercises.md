...
原生库来自选定 NVIDIA Ubuntu deb 安装，该安装没有 Toolkit `version.json`。增加基于 EX21 五项 Toolkit 包、独立固定的驱动用户态包和文件所属包的配置身份阶段，不伪造文件，也不引入 runfile 回退。题设不提供驱动用户态加载或导入成功记录。

同时保留 EX21 的禁用缓存配置：默认 NVRTC 缓存可以在未构造 Device 时独立调用 `cuInit()`。说明在哪里设置 `CUDA_CACHE_DISABLE=1`、哪些 core 选项关闭缓存，不能只因没有 GPU 启动就推断没有初始化。
...
对规范 CLI 的显式清理，另行区分 Python stderr/FD 2 捕获与最终析构覆盖。保留诊断载荷有界，不代表临时磁盘有界，也不保证能观察到判定后 GC/解释器关闭才发出的输出。
