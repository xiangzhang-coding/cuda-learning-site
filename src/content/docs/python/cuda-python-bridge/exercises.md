...
原生账本使用 EX21 固定的五项 NVIDIA Ubuntu Toolkit deb 记录、独立的驱动用户态包记录，以及解析后编译器/链接器文件的所属包。解释为何缺少 Toolkit `version.json` 不能成为拒绝这一安装方式或自行补文件的理由。区分已安装包版本、API 主/次版本查询与实际库哈希；题设不提供 runfile 替代方案，也不提供导入成功记录。
