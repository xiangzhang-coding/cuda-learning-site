...
**迁移任务：** 把同一份 CUDA C++ 责任标注到 Python 方案：`cudaSetDevice`、`cudaStreamCreateWithFlags`、`cudaMalloc` 或 `cudaMallocAsync`、`cudaMallocHost`、`cudaMemcpyAsync`、带类型的 C++ 核函数启动，以及 CPU 比较前的 `cudaStreamSynchronize`。表格列出 C++ API/句柄、core 调用或对象、持有者/最后使用，以及一项不能机械替换的语义差别。另行对照 Runtime 管理的核函数身份、Driver 的 `CUmodule` / `CUfunction` 与 core 的 Kernel/库所有权。不实现假设核函数，也不修改 EX21。

**预期提交证据：** 修复每项字节数、指针/标量表示、启动维度、复制目标与视图所有权。求前五项输出、有效/尾部线程数，以及按存储位置区分的载荷总数。分别说明 n=0、负 n 和超出 uint32 范围的处理。提交 CUDA C++ 责任到 Python 的对照表，填写本请求的具体字节数、标量位宽与完成点，不能只列 Python API 名称。

**验收标准：** 每个逻辑索引恰好有一个写线程，尾部不访问数组。传输双方 Buffer 等长且主机目标明确。只在主机可访问、持有者仍存活的存储上创建视图。不能把普通 Python 标量封送当成签名匹配。`s.sync()` 后比较每个有限输出，不能用五个样本匹配替代全量运行判定。迁移表须保留 C++ 的所有权/完成责任，并明确说明主上下文选择、分配器回退、整 Buffer 复制范围、核函数句柄所有权和 Python 定宽标量为何不是一对一 Runtime 调用。
...
**迁移任务：** C++ 持有者通常以 `cudaFree` / `cudaFreeAsync` 或 `cudaFreeHost` 配对释放分配，以 `cudaStreamDestroy` 配对释放流；独立拥有的 Driver retain 或模块另有释放责任。标注哪些责任在 core 方案中保持不变，哪些原生释放权限不会转给借用者。区分平衡独立取得的 `CUcontext` retain 与重置共享主上下文。

**预期提交证据：** 分类已取得、借用与共享资源，纠正 ctx 的含义，画正常完成路径及完成失败路径。指出应保留的原始错误和次要诊断，包括原生 stderr 及 Python 异常/警告渠道。说明哪些资源有公开 close、何时可调用，为何清理不能在完成失败后建立有效结果，以及修复顺序如何把 C++ 最后使用责任带到 Python。

同时标注捕获边界：显式 `cleanup_step` 期间发出的诊断属于该动作判定，延至解释器关闭才执行的析构则在边界外。说明保留载荷的 16,384 字节限制约束什么、不约束什么，以及为何捕获干净不能认证之后的 GC 或最终全部清理。

**验收标准：** 不先于依赖流的 Buffer 释放关闭流，不用原始句柄重复释放，不卸载 core 拥有的库，不重置共享主状态。用 `sync()`，不用 `synchronize()`。保留失败并非零退出；清理诊断和失败等待不能转换为验收通过。不能假设 `del` 保证立即销毁原生资源，也不能认为每个原生清理诊断都会成为 Python 异常。解释 C++ 持有者的释放调用，与 core 自有 Buffer/Stream 关闭、Kernel/ObjectCode 共享生命周期及主上下文持有之间的差别。
