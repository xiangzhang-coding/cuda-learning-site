...
const [releaseManifest, publicationManifest] = await Promise.all(
  ['src/r5-release-manifest.json', 'src/current-publication-manifest.json'].map(async (relativePath) =>
...
if (releaseManifest.releaseId !== 'R5' || releaseManifest.schemaVersion !== 6) {
  throw new Error('The source release manifest is not the reviewed R5 schema.');
}
if (
  publicationManifest.publicationId !== 'current' ||
  publicationManifest.schemaVersion !== 1 ||
  publicationManifest.releaseReview?.latestCompleted !== 'R5' ||
  publicationManifest.releaseReview?.next !== 'R6' ||
