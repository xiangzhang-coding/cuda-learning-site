...
const [releaseManifest, publicationManifest] = await Promise.all(
  ['src/r4-release-manifest.json', 'src/current-publication-manifest.json'].map(async (relativePath) =>
...
if (releaseManifest.releaseId !== 'R4' || releaseManifest.schemaVersion !== 5) {
  throw new Error('The source release manifest is not the reviewed R4 schema.');
}
if (
  publicationManifest.publicationId !== 'current' ||
  publicationManifest.schemaVersion !== 1 ||
  publicationManifest.releaseReview?.latestCompleted !== 'R4' ||
  publicationManifest.releaseReview?.next !== 'R5' ||
