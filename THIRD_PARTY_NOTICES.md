...
The new softmax Lab kernel, scalar oracle, harness and teaching materials are
...
The 2026-09-13 #45 increment adds reference-only PyTorch SDPA/profiler source
review under the exact root BSD-3-Clause LICENSE and NOTICE. Its per-file
hashes and ancestry are in SRC-CUDA-086 and
[CONTENT_LICENSES.md](CONTENT_LICENSES.md#optimization-and-sdpa-reference-only-review).
No upstream implementation, test, trace, binary or timing is redistributed.
The existing external PyTorch 2.11.0+cu128 and NVIDIA component notices below
continue to govern LAB14's independently provisioned environment.
...
The production output carries this notice plus the complete project license texts and the exact Astro, Starlight, and Pagefind license notices for interfaces bundled into the static site. The sole adapted upstream source file is the MIT Search override below; no upstream documentation passage, diagram, or sample code is copied or adapted into instructional content. Exact transitive versions, sources, integrity values, declared licenses, install scripts, and bundled flags are reviewed from the committed npm lockfile by the repository quality checks.

## Adapted Starlight Search

Accessed **2026-09-13**: `src/components/Search.astro` adapts [`packages/starlight/components/Search.astro`](https://github.com/withastro/starlight/blob/656ffd54e5b27483f542c9eb8b12fd32f44372ae/packages/starlight/components/Search.astro), tag `@astrojs/starlight@0.41.7`, commit `656ffd54e5b27483f542c9eb8b12fd32f44372ae`. It retains MIT and `Copyright (c) 2023 [Astro contributors](https://github.com/withastro/starlight/graphs/contributors)`, with the complete upstream [LICENSE](https://github.com/withastro/starlight/blob/656ffd54e5b27483f542c9eb8b12fd32f44372ae/LICENSE) text in its source frontmatter and existing `legal/starlight-0.41.7-MIT.txt` distribution. There is no upstream NOTICE file. The [exact-file ledger](CONTENT_LICENSES.md#starlight-search-override) records source/license blob identity and the narrow changes: public imports, single-flight intent/idle initialization, late DOM readiness, focus and localized bounded failure. Project Apache-2.0 and CC BY 4.0 licenses do not replace this file's MIT terms. This new dated adaptation does not rewrite the R4 review above.
