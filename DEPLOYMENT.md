...
The rolling `src/current-publication-manifest.json` and its `/publication.json` output include L01-L11, EX17/EX18, and LAB11/LAB12: 73 Learning Units, 18 Runnable Examples, 12 Labs, 19 Visual Explainers, 78 Practice Bank entries, 192 Glossary terms, 88 source records, 389 catalog records, 72 Exercise-set and 72 solution-set Publication Pairs, 269 Publication Pairs, and 538 source routes. Issue #38 adds complete L10/L11 Publication Pairs and their Exercises and separate reviewed solutions; L12-L13 and the R4 aggregate review remain pending. This publication-inventory-only update is dated 2026-09-07, not a new API or infrastructure review, deployment, or acceptance claim; existing source-item dates remain unchanged. Independent cuDNN 9.24.0 and frontend 1.27.0 pins are reference-only and add no build or runtime requirement to deployment. Issue #32 retains dynamic R3 acceptance.
...
Before accepting production, require successful `web-quality` and `cuda-compile-gate` checks for the same `main` commit; source text does not claim those checks have run. EX02, EX10, and LAB02 retain Compile-Checked evidence. EX11-EX18 retain empty compilation evidence and Pending Hardware Verification where runtime applies. LAB09-LAB12 have empty compilation and recorded-observation arrays and remain Pending Hardware Verification. Q06-Q13, A10-A14, and current L01-L11 are Learning Units with all four evidence arrays empty and grant no Evidence Status. EX17's five-profile matrix and the LAB11 compile path are non-evidentiary build gates, not qualifying compilation evidence. EX10 is Runtime-Not-Applicable. No Reference Environment, Community-Observed subject, Runtime-Verified subject, captured profiler report, or performance observation is declared.
...
For a Preview URL, set `RELEASE_BASE_URL` and `RELEASE_KIND="preview"`. The gate must check that `/release.json` remains the immutable R3 contract with 232 Publication Pairs, 464 source routes, and 347 catalog records while `/publication.json` matches the rolling current boundary with 269 Publication Pairs, 538 source routes, and 389 catalog records. Required coverage includes both locales, all 538 routes, navigation/search, current catalog counts, the complete current graph including L01-L11, EX17/EX18, and LAB11/LAB12, the exact immutable ten-entry R3 report-analysis inventory, six current expected-only profiler plans, empty evidence arrays, legal notices, canonical metadata, and browser/network errors. These are acceptance requirements, not a claim that the current publication has passed a remote gate.
