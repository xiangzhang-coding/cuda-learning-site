...
For a Preview URL, set `RELEASE_BASE_URL` and `RELEASE_KIND="preview"`. The gate checks that `/release.json` remains the immutable R3 contract with 232 Publication Pairs, 464 source routes, and 347 catalog records while `/publication.json` matches the rolling current boundary with 249 Publication Pairs, 498 source routes, and 370 catalog records. It covers both locales, all 498 routes, navigation/search, current catalog counts, the complete current graph including L01-L05, EX17, and LAB11, the exact immutable ten-entry R3 report-analysis inventory, six current expected-only profiler plans, empty evidence arrays, legal notices, canonical metadata, and browser/network errors.
