...
Alongside those ten entries, R4 qualifies exactly eight library-and-algorithm-choice entries: `PB-R4-001`, `PB-R4-002`, `PB-R4-003`, `PB-R4-004`, `PB-R4-008`, `PB-R4-011`, `PB-R4-012`, and `PB-R4-016`. They are already part of the 82-entry inventory. R4 does not certify framework or Triton track completion and adds no placeholder destinations for R5.
...
Before accepting production, require successful `web-quality` and `cuda-compile-gate` checks for the same `main` commit; source text does not claim those checks have run. EX02, EX10, and LAB02 retain Compile-Checked evidence. EX11-EX20 retain empty compilation evidence and Pending Hardware Verification where runtime applies. LAB09-LAB12 have empty compilation and recorded-observation arrays and remain Pending Hardware Verification. Q06-Q13, A10-A14, and current L01-L13 are Learning Units with all four evidence arrays empty and grant no Evidence Status. EX17's five-profile matrix, the LAB11 compile path, and EX19/EX20's three-profile C++17 build jobs are non-evidentiary build gates, not qualifying compilation evidence. EX10 is Runtime-Not-Applicable. No Reference Environment, Community-Observed subject, Runtime-Verified subject, captured profiler report, or performance observation is declared.

The complete R4 runtime tally is 31 Pending Hardware Verification subjects: 19 Runnable Examples (all except EX10) and all 12 Labs. Only EX02, EX10, and LAB02 have qualifying Compile-Checked evidence. Six expected-only profiler plans retain unfilled Environment Manifests and empty recorded observations. Source/license review and deployment checks never become GPU evidence.
...
For a Preview URL, set `RELEASE_BASE_URL` and `RELEASE_KIND="preview"`. The gate must compare `/release.json` with the immutable schema 5 R4 contract and `/publication.json` with the exact current manifest, both currently containing 277 Publication Pairs, 554 source routes, and 401 catalog records and naming the same source commit. Required coverage includes both locales, all 554 routes, navigation/search, current catalog counts, the complete graph including L01-L13, EX17-EX20, and LAB11/LAB12, the exact ten-entry R3 report-analysis and eight-entry R4 library-choice inventories, six expected-only profiler plans, unchanged evidence, legal notices, canonical metadata, and browser/network errors. EX19 and EX20's downloaded immutable archives must match their local source, host-test, build-contract, and project-manifest bytes; a SHA-shaped link alone is insufficient. These are acceptance requirements, not a claim that R4 has passed a remote gate.
...
Issue #41 must record R4 acceptance separately, including exact check runs, deployed versions, Preview and production URLs, source commit, and remote smoke results. The 2026-09-10 aggregate source review supplies none of those dynamic coordinates.
