...
The production Learning Site origin is <https://cuda-learning-site.hmzhangxiang.workers.dev>. R3 is the latest completed aggregate review. Its immutable snapshot contains 62 Learning Units, 16 Runnable Examples, 10 Labs, 19 Visual Explainers, 66 Practice Bank entries including 10 Nsight report-analysis Practice Bank entries, 176 Glossary terms, 76 source records, 347 catalog records, 61 Exercise-set Publication Pairs and 61 solution-set Publication Pairs, 232 Publication Pairs and 464 source routes; `src/r3-release-manifest.json` emits that contract as `/release.json`.

The rolling `src/current-publication-manifest.json` and its `/publication.json` output include L01-L13, EX17-EX20, and LAB11/LAB12: 75 Learning Units, 20 Runnable Examples, 12 Labs, 19 Visual Explainers, 82 Practice Bank entries, 196 Glossary terms, 92 source records, 401 catalog records, 74 Exercise-set and 74 solution-set Publication Pairs, 277 Publication Pairs, and 554 source routes. Issue #40 adds L13, its Exercises and separate reviewed solutions, and EX20 as four complete Publication Pairs; L01-L13 are published and the R4 aggregate review remains pending. This publication-inventory-only update is dated 2026-09-09, not a new API or infrastructure review, deployment, or acceptance claim; existing source-item dates remain unchanged. Independent cuDNN 9.24.0 and frontend 1.27.0 pins are reference-only and add no build or runtime requirement to deployment. Issue #32 retains dynamic R3 acceptance.
...
The R3 scope is 62 Learning Units O01-O08/F01-F08/M01-M19/A01-A14/Q01-Q13, sixteen Runnable Examples EX01-EX16, ten Labs LAB01-LAB10, and nineteen Visual Explainers: standalone VIS01-VIS14/VIS18 plus embedded VIS19-VIS22. It also contains 61 Exercise-set Publication Pairs and 61 solution-set Publication Pairs, 66 Practice Bank entries, 176 Glossary terms, and 76 source records. The five catalog groups total 347 records; bilingual pages total 232 Publication Pairs and 464 source routes. The release identifies exactly 10 Nsight report-analysis Practice Bank entries: PB-R3-002 through PB-R3-005 and PB-R3-007 through PB-R3-012. L03/LAB11, L06/LAB12, and L13/EX20 remain absent from this immutable R3 snapshot; rolling issue #34 publishes L03/LAB11 without changing that history. Rolling issue #36 publishes L06/L07/EX18/LAB12, with LAB12 requiring Q13 and L06; it does not expand the R3 snapshot. Rolling issue #40 publishes L13/EX20 without changing R3 or its acceptance.
...
For a Preview URL, set `RELEASE_BASE_URL` and `RELEASE_KIND="preview"`. The gate must check that `/release.json` remains the immutable R3 contract with 232 Publication Pairs, 464 source routes, and 347 catalog records while `/publication.json` matches the rolling current boundary with 277 Publication Pairs, 554 source routes, and 401 catalog records. Required coverage includes both locales, all 554 routes, navigation/search, current catalog counts, the complete current graph including L01-L13, EX17-EX20, and LAB11/LAB12, the exact immutable ten-entry R3 report-analysis inventory, six current expected-only profiler plans, empty evidence arrays, legal notices, canonical metadata, and browser/network errors. EX19 and EX20's downloaded immutable archives must match their local source, host-test, build-contract, and project-manifest bytes; a SHA-shaped link alone is insufficient. These are acceptance requirements, not a claim that the current publication has passed a remote gate.
