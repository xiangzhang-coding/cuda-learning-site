...
The rolling `src/current-publication-manifest.json` and its `/publication.json` output include L01-L12, EX17/EX18/EX19, and LAB11/LAB12: 74 Learning Units, 19 Runnable Examples, 12 Labs, 19 Visual Explainers, 80 Practice Bank entries, 194 Glossary terms, 90 source records, 395 catalog records, 73 Exercise-set and 73 solution-set Publication Pairs, 273 Publication Pairs, and 546 source routes. Issue #39 adds L12, its Exercises and separate reviewed solutions, and EX19 as four complete Publication Pairs; L13 and the R4 aggregate review remain pending. This publication-inventory-only update is dated 2026-09-08, not a new API or infrastructure review, deployment, or acceptance claim; existing source-item dates remain unchanged. Independent cuDNN 9.24.0 and frontend 1.27.0 pins are reference-only and add no build or runtime requirement to deployment. Issue #32 retains dynamic R3 acceptance.
...
The R3 scope is 62 Learning Units O01-O08/F01-F08/M01-M19/A01-A14/Q01-Q13, sixteen Runnable Examples EX01-EX16, ten Labs LAB01-LAB10, and nineteen Visual Explainers: standalone VIS01-VIS14/VIS18 plus embedded VIS19-VIS22. It also contains 61 Exercise-set Publication Pairs and 61 solution-set Publication Pairs, 66 Practice Bank entries, 176 Glossary terms, and 76 source records. The five catalog groups total 347 records; bilingual pages total 232 Publication Pairs and 464 source routes. The release identifies exactly 10 Nsight report-analysis Practice Bank entries: PB-R3-002 through PB-R3-005 and PB-R3-007 through PB-R3-012. L03/LAB11, L06/LAB12, and L13/EX20 remain absent from this immutable R3 snapshot; rolling issue #34 publishes L03/LAB11 without changing that history. Rolling issue #36 publishes L06/L07/EX18/LAB12, with LAB12 requiring Q13 and L06; it does not expand the R3 snapshot. EX20 still waits for L13's exact cuSPARSE API contract.
...
Before accepting production, require successful `web-quality` and `cuda-compile-gate` checks for the same `main` commit; source text does not claim those checks have run. EX02, EX10, and LAB02 retain Compile-Checked evidence. EX11-EX19 retain empty compilation evidence and Pending Hardware Verification where runtime applies. LAB09-LAB12 have empty compilation and recorded-observation arrays and remain Pending Hardware Verification. Q06-Q13, A10-A14, and current L01-L12 are Learning Units with all four evidence arrays empty and grant no Evidence Status. EX17's five-profile matrix, the LAB11 compile path, and EX19's three-profile C++17 `ex19-build` job are non-evidentiary build gates, not qualifying compilation evidence. EX10 is Runtime-Not-Applicable. No Reference Environment, Community-Observed subject, Runtime-Verified subject, captured profiler report, or performance observation is declared.
...
For a Preview URL, set `RELEASE_BASE_URL` and `RELEASE_KIND="preview"`. The gate must check that `/release.json` remains the immutable R3 contract with 232 Publication Pairs, 464 source routes, and 347 catalog records while `/publication.json` matches the rolling current boundary with 273 Publication Pairs, 546 source routes, and 395 catalog records. Required coverage includes both locales, all 546 routes, navigation/search, current catalog counts, the complete current graph including L01-L12, EX17/EX18/EX19, and LAB11/LAB12, the exact immutable ten-entry R3 report-analysis inventory, six current expected-only profiler plans, empty evidence arrays, legal notices, canonical metadata, and browser/network errors. EX19's downloaded immutable archive must match the local source, host-test, build-contract, and project-manifest bytes; a SHA-shaped link alone is insufficient. These are acceptance requirements, not a claim that the current publication has passed a remote gate.
